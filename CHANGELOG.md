...
- **Verified local backups and recovery (PR #689)** - Settings Data and startup recovery now inspect physical storage compatibility, retain verified backups including committed WAL contents, and restore through a resumable operation with retained originals. Missing or unsupported Stores preserve data; compatible notes remain available during scoped Agent failures. Historical Tasks, Goals, schedules, delegation and Memory/Profile publications retain their evidence without regaining execution authority, while fresh work keeps its own admission. Settings waits for configuration readiness and refreshes open Models after Retry. The main review closed all thirteen findings with production-owner fixtures, crash/restart probes, focused tests and real Electron/visual checks. The earlier full Core run retains one base-reproduced legacy-residue failure; final-head E2E samples were still running at merge. Current behavior is in the data lifecycle spec. Automated release-fixture enforcement remains a separate draft feature; no released-version upgrade or physical power-loss certification is claimed.

- **Unreadable conversations now have targeted recovery (PR #687)** - startup issues can inspect an exact catalog closure, rebuild from a verified Rollout or retained projection, or remove only a confirmed closure after retaining and verifying owner evidence. Recovery coordinates Profile/Memory, scheduled tasks, Delegation, Tool Tasks, resources, publication records and WAL-aware SQLite snapshots through one restartable fenced operation; stale scope, live work, unknown ownership and retention failure preserve active data. Recovered histories carry explicit provenance, retained originals become available only after completion, and shared attachments, Projects, Memory Nodes and accepted Profile content remain protected. Verified with typecheck, focused recovery/ThreadService/renderer tests, `docs:check`, and the final review regressions.

- **Profile files now provide direct personal context and editable Agent identity (PR #686)** - selected Profiles use `IDENTITY.md` and `STYLE.md`, while global `USER.md` stores scoped personal entries with authorship, source provenance, revision checks, interruption recovery and explicit Reset/rollback invalidation. Eligible Memory extraction routes stable preferences directly to the Profile owner; canonical Thread context appends only changed keyed state or named withdrawals, preserving earlier provider messages and cache boundaries. Settings and ordinary file tools share the same publication owner, and private recovery snapshots/publication bodies follow bounded retention cleanup. Verified with typecheck, `docs:check`, focused Profile/Memory/context tests, renderer tests and the branch E2E comparison; no branch-specific E2E regression was reported.

- **Memory retains useful source-aware records and names completed days (PR #685)** - eligible conversations now process oldest complete evidence in bounded batches, preserve exact accepted coverage and independent support across reader, assistant, tool, web and MCP sources, and retain external knowledge with attribution. Personal subjects require current reader-authored text through extraction and consolidation; detached snapshot planning, bounded unsupported-subtree cleanup, durable continuation recovery, and source-aware admission protect publication. Daily containers start as `Memory #mem-day` and receive a completed-day title only after source readiness and full-subtree checks pass; manual titles, edits, focus, folding and scroll remain authoritative. Verified with typecheck, build, `docs:check`, 156 Memory/Reset/Outline tests, 169 Memory-operation/ThreadService tests, two real-Electron Memory smokes and light/dark inspection. No live-provider quality or cost claim is made; the Node retention plan is archived and Profile files/direct learning remains a separate unit.

- **Task status now exposes attributable execution facts (PR #684)** - Agents receive the recorded source Turn/Item, admitted directory, start/completion times, supervisor/child PIDs and a separate state observation timestamp. Missing inspection facts remain null; matching paths, ports or PIDs do not establish ownership, and equal hashes or recovered receipts do not prove an effect never occurred. The design from #680 is folded into current specifications and archived with all three units delivered.

- **Projects and conversation work folders (PR #674)** - Projects retain up to 20 source folders and an explicit primary; new chats copy that primary, while existing chats and forks own independent saved defaults. UI controls and invocation-bound CLI/Skill operations share revision checks, native Project review and durable receipts; confirmation-free folder changes are restricted to the invoking conversation. Missing or redirected folders retain their identity and block dependent calls without fallback. Composer/history labels expose location details and compact model/effort identity. The P1 authorization finding is resolved. Gate checks passed typecheck, docs:check, 25 final Project tests, the bundled CLI integration case, and six light/dark UI tests on the preceding UI-identical head. The source CLI case failed twice during cleanup with EBADF; the preceding PR head passed the same pair, and the cause remains open. The PM requested merge with that limitation recorded; remote E2E samples were still pending. The Project payload shape requires fresh isolated pre-release data without a legacy reader. Current specs contain the delivered design and the plan is archived.

- **Tool Tasks now expose actual process isolation (PR #658)** - Task details, status, retained receipts, and bounded model-context observations distinguish pending activation from sandboxed, unsandboxed, unavailable, or rejected execution. A private acknowledgement proves macOS profile activation while the trusted supervisor retains output and receipt ownership; required isolation never falls back to unrestricted execution, and network access remains explicitly unrestricted. The development Skill and reproducible tmux probe record detached-process, directory-claim, pane-output, and isolated-pane limitations without introducing a persistent terminal. Task storage and receipt formats require fresh userData under the pre-release cutover policy. Gate verification passed typecheck, `docs:check`, 263 focused tests, seven compiled-supervisor checks under Electron, two light/dark browser tests, and the production build. A combined test run exited 137; isolated batches passed. The gate could not rerun tmux because it was unavailable locally; its source/Electron measurements are recorded in the specification. Remote E2E sampling was still queued/running at merge. Unit E and the complete A-E workbench series are folded into current specs and archived.

- **Reviewed Git changes can now be committed and explicitly published (PR #657)** - immutable captures bind selected working files to exact HEAD/ref, index, and content evidence. Commits preserve unrelated staged and unstaged files; explicit remote previews and reconciliation support push and same-repository GitHub PR creation. Historical results offer unchecked file selection and a copyable commit request. Read-only inspection avoids repository scripts, and large reviews remain committable through bounded stdin with a Host digest binding. Both P2 findings were resolved. Gate verification passed typecheck, `docs:check`, 185 focused tests in isolated batches, four independent reproductions, light/dark browser verification, production build, and a compiled-helper smoke under Electron's Node runtime; initial parallel test timeouts and exit 137 are recorded in the PR. Remote E2E sampling was still running at the merge check. Unit D is complete, its design is folded into the specs and archived, and workbench E remains active.
...
- **Application and diagnostics operations now share one Host owner for people and root Agents (PR #650)** - bounded tools expose build facts, bundled release notes, cached update state, fresh update checks, validated release/download links, fixed support destinations, and local diagnostic counts/reveal/export. About consumes the same bundled-release owner; native export revalidates cancellation, authority, and Host/window lifetime before writing. Domain failures retain semantic failure outcomes, and all four tools enforce closed, bounded output contracts. The gate passed typecheck, `docs:check`, 47 focused Core/Renderer tests, and whitespace checks; the branch also reports passing Settings E2E, application smoke, Renderer suite, and packaged build. Five GitHub E2E samples completed with no introduced failures after baseline comparison; existing Core/smoke failures are recorded in the PR.

- **Task-owned workbench execution context now spans local tools, delegation, Automation, and Thread rendering (PR #646)** - executable tasks persist admitted addresses, capability/isolation policy, context evidence, receipts, and recovery state before side effects. Canonical context publication and compaction recovery preserve scoped baselines and pending evidence, while configuration-source ownership, native launchers, Automation dispatch, and renderer task details use the same contracts. The former persisted Thread working directory and startup injection paths are retired under the pre-release fresh-data policy. Verified with typecheck, `docs:check`, 2,777 Core tests (6 skipped), 1,463 renderer tests, focused execution-context/local-tool/kernel coverage, and the merge-conflict Memory renderer suite.

- **Memory operations now have one bounded path for people and root Agents (PR #647)** - `memory_inspect` and `memory_manage` share Host-owned status, saved-search opening, native-confirmed exact-target Reset, and revision-guarded per-Thread mode changes. Reset captures canonical containers and all descendants, revalidates the reviewed target before deletion, records durable prepared/finalized/conflicted/unknown settlement, and recovers without duplicate deletion; global enablement remains the file-backed `agent.memory.enabled` setting. Verified with typecheck, `docs:check`, focused Core/renderer/Memory coverage, real-Electron Memory smoke, whitespace checks, and the existing baseline failures recorded separately.

- **Preview translation and data operations are now preview-local and Host-owned (PR #648)** - each live preview keeps its own language, model, automatic-translation, and display intent with revision-checked renderer acknowledgement; root Agents and Settings share bounded inspection and native-confirmed clearing for saved translations and the preview website partition. Translation cache writes are fenced across content/global clears, website cleanup reports per-step outcomes and reload state, and the former global translation preferences and renderer singleton stores are retired under the pre-release fresh-data policy. Verified with typecheck, `docs:check`, focused Core/renderer preview coverage, complete Core and Renderer suites, GitHub E2E samples, and whitespace checks.

- **Skill lifecycle operations now have one Host-owned path for people and Agents (PR #644)** - root-only `skill_inspect` and `skill_manage` cover discovery, review, install, update, rollback, uninstall, and governed one-step Agent edit undo. Mutations bind the reviewed source and exact revision/content hashes, recheck Turn and capability authority before commit, and report committed content separately from observed availability and runtime refresh. The old managed enabled flag and raw renderer mutation routes are retired; public file-backed settings remain the availability authority. Verified with typecheck, build, `docs:check`, focused lifecycle/service/renderer coverage, and native Electron smoke tests; the managed index schema changes from 2 to 3 without a migration reader under the pre-release reset policy.

- **Skill declarative settings now have a domain-owned route (PR #643)** - Skill source bindings and disabled identities use dedicated file-backed read/update commands, refresh the active runtime, and preserve stored relative/home-relative source paths when the Skill Library sends canonical paths back. Refresh fencing covers queued writes through success or failure settlement so delayed settings reads cannot undo toggles. This completes declarative settings ownership; Unit D's Agent-facing lifecycle and provenance operations remain open. Gate verification passed typecheck, `docs:check`, 124 focused Core/Renderer tests, two independent refresh-race reproductions, and whitespace checks; the preceding head passed 27 targeted Settings/cursor E2E tests and light/dark visual checks, and the final head's five E2E samples introduced no failures beyond the `main` baseline.
...
- **Workbench development now uses native commands and UI-owned Projects (PR #660)** - Git/GitHub and verification Skills replace the intercepted Git protocol, private source/check verification engine, Project model tools, and specialized review card. Background Tasks no longer reserve directories; validated parent/child execution ownership, scheduler limits, Goal budgets, generic receipts/artifacts, and actual process isolation remain. Native publication reconciles every actual push URL, including partial success, and literal-path inspection works under read-only delegation. Both P2 review findings were fixed and independently verified. Gate checks passed typecheck, `docs:check`, build, focused Core/renderer suites, three light/dark browser cases, and a fresh-storage Electron smoke; the final fix passed 85 targeted tests. One concurrent test timeout passed on isolated rerun; five existing environment-dependent tests were skipped and remote E2E sampling remained queued at merge. This pre-release format cut requires fresh isolated userData without migration or automatic deletion. Current specs and recovery/records dependencies are reconciled, and the plan is archived.

- **Settings now unifies discovery, configuration and maintenance (PR #656)** - one native window provides searchable categories, history, source repair and an Advanced inspector while retaining pane drafts and pending operations. Focused Provider sheets keep credentials masked until explicit Show/Copy; Agent edits preserve exact capability restrictions and retain drafts through source-conflict retries. Twelve Settings management/inspection tools are retired: Agents edit public configuration files with existing file tools and the configuration Skill, while maintenance stays in UI-owned services with native confirmations. The remaining P2 was fixed and independently verified. Final-head checks passed typecheck, `docs:check`, build, 27 focused Core tests, all 1,476 renderer tests, nine Agent browser cases and two native user/project conflict cases. The preceding deep pass covered 97 browser cases, all 41 Provider sheets and light/dark enlarged-text layouts; initial process/PDF and native-close failures passed isolated reruns. The final catalog assertion passes at 20 tools; full Core was not rerun for the final remediation, and non-gating remote E2E sampling remained queued/running at merge. Current specs are synchronized, the discovery and A-G aggregate plans are archived, and startup fault isolation is eligible; the separate Settings WorkingText consumer remains planned.

### Fixed

- **Task control now rejects incorrect action parameters consistently (PR #681)** - Closed nested request alternatives and exact admission share one action definition across provider schemas. Bounded errors explain repairs without echoing rejected keys or values, and malformed input never executes. Gate review resolved reordered-key receipt replay and empty-key admission regressions while preserving stored digests and rejection of genuinely changed input. Final admission/native/Task suites passed 109 tests plus three independent reviewer regressions; the complete ThreadService suite passed 165 tests, including both Project CLI paths. Typecheck, catalog and document checks passed. An earlier Project CLI authority failure passed a focused rerun and the final full ThreadService run; its cause remains unexplained.

- **Task recovery follows operation outcomes (PR #683)** - Accepted, conflicting and already-handled receipts now provide distinct next steps; status requests acknowledgement only for a pending event. Receipt acceptance, command exit and application availability remain separate facts, without a new retry owner or inferred restart authority. The final combined build and real-Electron smoke passed, covering handoff, attributable status, restart, silent service exits and exactly-once finite delivery; live-model measurements remain scoped to the author-reported samples, not a universal compliance guarantee.

- **Agent images retain source pixels at the model boundary (PR #676)** - attachments, file reads and tool/generated images share serialized byte decoding, bounded resizing and canonical encoding instead of OS thumbnails. PNG transparency, all eight JPEG EXIF orientations, geometry, immutable originals and replay are preserved, including `.blob` storage. Review found no actionable issues; real-Electron upload/provider/restart fixtures passed. Integrated over #678 with #677: typecheck, build, docs check, 202 focused Core tests (5 environment skips), and all 6 image/service/isolation Electron smoke tests passed. The four Task-authority and two Project CLI cases passed, with one initial bundled-CLI authority-timing failure passing on isolated rerun; the failure is retained in the gate evidence. Full Core/browser E2E and live-provider acceptance were not rerun; non-gating remote E2E samples remained pending.

- **Service handoff accepts owned checks across execution directories (PR #677)** - successful post-launch Item evidence retains its independently admitted address while Task ownership, target liveness, revisions, Stop and receipt replay remain authoritative. Expected refusals carry specific recovery codes. The development Skill verifies the exact application's Runtime/content roots without auto-starting it, preserves IPv6 endpoints, and reports partial startup when required dependencies fail. The review's false-positive timeout fixture is corrected: a real child starts, remains live through the exact timeout, and exits through fixture-owned cleanup. All four inspection tests and combined integration checks above passed. The original dev-session Runtime timeout remains unexplained; neither successful frontend checks nor isolated failure-channel fixtures establish its cause.

- **Private-control Task cleanup preserves open Host files (PR #678)** - Host-to-supervisor control now uses standard stdin, avoiding Bun's extra-pipe descriptor ownership failure when a closed descriptor is reused by a live Host file. The actual command retains separate literal user stdin and its fd 3 capability; authorization, environment filtering and persistence remain unchanged. This closes #674's source CLI cleanup tail without suppressing Rollout close errors. Gate review found no actionable issues; typecheck, `docs:check`, all 50 ToolTask tests (including forced-GC descriptor reuse), and both source/bundled Project CLI cases passed with normal cleanup. Electron smoke was not rerun at the gate, and five non-gating E2E samples remained pending. No fix to the separate Outline Runtime timeout or the author's isolated authority-timing failure is claimed.

- **Handed-over applications no longer trigger unsolicited Agent replies when they exit (PR #673)** - verified service handoff ends the launch obligation while preserving the process, logs, isolation and Stop control. Finite background jobs still deliver their result once; an explicitly requested watch remains independent. Task-owned receipts and terminal-event disposition survive restart. Stop cancels only the addressed pending responsibility; acknowledgement binds that result to its current handling Turn. Stopping an already finished delegated invocation now preserves its released Session, any successor and committed handling Turn. Task details separate process outcome, verified launch, watch and known Stop source; silent service failures retain visible facts. The P2 review finding is fixed. Final verification passed typecheck, `docs:check`, 245 focused tests and the original independent reproducer; earlier review passed 517 Core tests (five environment skips), 35 renderer tests, three browser cases and real-Electron handoff/restart/exit smoke with light/dark inspection. Six CI entries labelled introduced also fail 5/5 on the exact base; their line-based attribution is tracked separately, and the full suite is not claimed green. Required Task continuation/control-receipt fields need fresh pre-release development data. Specs, archived design and downstream references are synchronized.

- **Questions recover reliably and preserve independent drafts (PR #672)** - authoritative snapshots and ordered notifications restore live questions after renderer reload or subscription loss. Requests default to a 60-second deadline and settle exactly once; timeout, cancellation and unanswered questions never invent an answer or permission. Step navigation retains answer drafts locally, final submission sends active answers and typed skips, and the ordinary message draft stays independent. Expiry preserves unsent text in the original conversation without reviving a historical request. All four review findings were resolved, including a Host-generation race during an outstanding snapshot read. Gate verification covered typecheck, `docs:check`, 394 focused unit tests, 28 question E2E cases, real-Electron loss/reload/timeout/restart smoke and light/dark views. The final fixes additionally passed 40 store/draft tests and two waiting-state E2E cases; existing broader-suite failures remain documented in the PR. Current specifications are updated, the plan is archived, and downstream scheduling/recovery designs consume the delivered lifecycle.

- **Startup failures preserve healthy notes and conversations (PR #664)** - the visible shell reports independent Outline/Agent availability, bounded copyable issues, native configuration-source inspection, and lifecycle Retry without resetting saved data. Failed Agent construction releases acquired stores, dependent producers remain fenced, and unreadable conversation histories identify their source and affected descendants. Healthy notes and chat drafts remain mounted through issue inspection; subscriptions retain background updates, and configuration-observation retry applies missed preferences before reporting recovery. Translation-cache filesystem failures preserve saved bytes. Both review findings were fixed and independently reproduced. Integration over #666/#667 passed typecheck, `docs:check`, whitespace checks, 227 focused Core tests, all 1,481 renderer tests, the Electron build, and 15 real-Electron startup/model-selection smoke tests; light/dark recovery screenshots were inspected. Full Core and full browser E2E suites were not completed at this gate; non-gating remote E2E samples remained queued. Current behavior is folded into the specs, the plan is archived, and subsequent recovery/Memory plans reference the final startup owners.

- **Default text model changes now save the chosen model (PR #666)** - the Models pane captures the selected value during the change event before queuing the write. React restoring the previous controlled value can no longer turn a concrete choice back into Automatic while reporting success. Consecutive choices preserve their order; failed saves retain the stored selection and allow retry. Gate review found no actionable issues. Integration over #667 passed typecheck, `docs:check`, whitespace checks, 20 focused unit tests, three browser regressions, and the Electron build. A real-Electron smoke verified configuration-file persistence, app restart recovery, and restoration to Automatic; light/dark screenshots were inspected. Two non-gating remote E2E samples passed and three remained running at merge.

- **Live shortcut changes survive delayed initial reads (PR #667)** - Keyboard Shortcuts subscribes before reading and ignores a pending initial read's success or failure after a live change arrives. The latest bindings and source digest remain available for subsequent edits; genuine initial failures still display and recover on a later notification. Gate review found no actionable issues. Typecheck, `docs:check`, whitespace checks, 39 focused unit tests, and seven shortcut browser tests passed, with light/dark screenshots inspected. IPC ordering is covered through renderer/browser mocks; native smoke was not rerun and non-gating remote E2E samples remained queued/running at merge.
