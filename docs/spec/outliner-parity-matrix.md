...
| Search outline | Render one always-available compact result-view band with name search, Table, Display, Group, Sort, and Filter; omit query chips, result count, manual refresh, and a second full toolbar. The title query action temporarily replaces the band with the query editor. | `search-query-builder.spec.ts`, `visualRows.test.ts`, `rowInteractions.test.ts` |
