...
| File node render | A non-image file (attachment) renders its row content as a uniform **file card**: a file-type icon (audio / video / pdf / generic), the filename on a single truncated line, a `type · size · pages/duration` meta line, and a `⋯` action menu. The filename is **display-only** (renamed on the node page, never inline); **clicking the card opens the file's node page** (its preview). An **image** is the one exception (see below). The bullet stays a plain node handle (the file-type icon lives on the card). |
...
| File node preview (node page) | Opening the file (a card click, the `⋯` menu, or drilling the bullet) shows it as a node page whose body is the preview "hero" — the rendered file with its meta + actions — above the node's children outline. This is the single full-preview surface (plus split-pane peek); there is no inline preview block in the outline. |
...
| Missing asset metadata | The node-page preview shows an unavailable message; the row stays an ordinary editable filename row and exposes no broken system actions. |
