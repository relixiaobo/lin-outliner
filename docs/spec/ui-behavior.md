...
| File node is a normal node | A file node behaves like any node: the chevron expands its **children** (not a preview), and a childless file node is a leaf. It can carry child notes, be moved, referenced, pinned, and opened in a split pane — all for free. The preview is not inline; it lives on the node page. The filename editor still mounts (visually hidden) so the row keeps full keyboard parity (arrow nav, Enter to add a sibling). |
...
| File node ⋯ menu | Both presentations carry a `⋯` action menu whose primary action is type-specific — **Maximize** for an image (opens the node page) and **Open in split** for other files (opens the node page in a split pane) — plus **Reveal in Finder** (acts on the stored asset). It reveals on hover and stays visible while open. |
