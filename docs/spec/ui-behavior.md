...
| File node render | A non-image file (attachment) renders its row content as a uniform **file card**: a file-type icon (audio / video / pdf / generic), the filename on a single truncated line, a `type · size · pages/duration` meta line, and a `⋯` action menu. The filename is **display-only**; **clicking the card opens the unified file preview surface** in its ingested state. An **image** is the one exception (see below). The bullet stays a plain node handle (the file-type icon lives on the card). |
| File node is a normal node | A file node behaves like any node: the chevron expands its **children** (not a preview), and a childless file node is a leaf. It can carry child notes, be moved, referenced, pinned, and opened in a split pane — all for free. The preview is not inline; it lives on the unified file preview surface. The row carries no inline text editor; a visually-hidden, focusable **keyboard anchor** (a lightweight element, not a read-only editor) gives the row full keyboard parity — arrow nav, Enter to add a sibling, Tab to indent, Backspace to remove the node. |
| File node row click & reference | A click on the card/image opens the file; a click in the empty area beside it selects the row (it never drops a caret into the hidden anchor). A `reference` whose target is a file node still renders as a normal **reference row**, not as the file's card/image. |
| File node preview | Opening the file (a card click, the `⋯` menu, or drilling the bullet) shows the unified file preview surface in its ingested state: outliner-ancestry breadcrumb, **read-only filename title**, preview "hero" (the rendered file with its meta + actions), then the node's children outline. The title prefers the node display text, then the stored original filename/source identity, so old file nodes with blank text do not show `Untitled`. This is the single full-preview surface (plus split-pane peek); there is no inline preview block in the outline. |
| Image render (inline) | An image is the one file kind that renders inline as the image **itself** (an image's content is its identity) instead of a file card: a bounded `<img>`, with no file-type icon and no filename in the row. Its `⋯` menu floats at the image's top-right, revealed on hover; clicking the image maximizes it (opens the ingested file preview). |
| File node ⋯ menu | Both presentations carry a `⋯` action menu whose first action is type-specific — **Maximize** for an image (opens the ingested file preview) and **Open in split** for other files (opens the ingested file preview in a split pane) — followed by the stored asset's system actions **Open**, **Reveal in Finder**, and **Copy file** (the same descriptor set the preview hero shows). It reveals on hover or keyboard focus and stays visible while open. |
| File node actions | The ingested preview hero carries Open / Reveal / Copy alongside `type · size · pages/duration` meta. Open uses the OS default app after the main process revalidates the asset path and local-file policy. Reveal shows the stored asset copy. Copy puts the stored asset file on the clipboard. |
| Missing asset metadata | The ingested preview shows an unavailable message; the row stays an ordinary file row with a display-only filename and exposes no broken system actions. |
| Non-node source preview | A source with no node (agent payload, loose inline local-file ref, url) opens the same `file-preview` surface in its loose state: source/path breadcrumb, read-only filename/source title, and the shared preview hero, but no children outline. |
