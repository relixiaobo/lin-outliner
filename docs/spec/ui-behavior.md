...
properties, unsupported chords, and conflicts leave the exact source untouched
and retain the preceding effective bindings. Main owns system registration and
webview guest interception; the main renderer receives only an effective
...
up to four alternates, removal, disable/enable, per-command Reset, Reset All,
and Open Keybindings File. Structured edits use the observed source digest and
preserve JSONC comments, ordering, whitespace, and unrelated properties. A
rejected source stays openable but disables structured edits. Failed global
registration shows the desired request and the previous binding still active.

Navigation, selection, editing, clipboard, undo/redo, printable input, and IME
grammar remain fixed in `src/renderer/ui/interactions/shortcutRegistry.ts`.
Configuring an application command changes only the explicit configurable
identity; it never rewrites those fixed definitions.
...
| Expanded scope guide line | Render as a visible neutral guide for expanded rows, including leaf rows expanded to show trailing input. Its resting 1 px stroke uses `--separator` and hover uses `--border-emphasis`, both without changing layout. The stroke is placed directly on its pixel-aligned offset without a transform, so image-height guides remain visible without darkening the ordinary guide color. Every marker has the same transparent marker slot; the visible glyph is only centered content inside that slot. The guide uses the actual marker slot DOM as its geometry source, not the visible glyph or an estimated flat-row layout: the flat renderer measures `.row-bullet-button` for the parent and structurally final mounted descendant relative to `.outliner-flat-guides`, then feeds measured `left` / `top` / `height` into the guide. A temporary incomplete measurement during preview resize, optimistic insertion, focus transfer, or virtualization keeps the prior geometry; only a structural collapse, removal, or definitive viewport exit removes it. Non-windowed resize measurement is isolated inside the guide overlay so it does not re-render ordinary rows. The band starts just below the parent marker slot so marker clicks remain owned by the marker itself. File icons, bullets, and other marker glyphs share one structural marker slot regardless of glyph size. The line ends on the last visible descendant marker centerline, so tall previews, wrapped content, and glyph size never stretch the structural line. |
