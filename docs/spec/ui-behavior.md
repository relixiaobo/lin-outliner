...
| File node render | A non-image file (attachment) renders as a **lightweight name row**: its **file-type icon is the bullet** (the `file` RowMarker variant — audio / video / pdf / generic), the row content is the **read-only filename** on a single truncated line, and a hover `⋯` action menu carries the asset actions. The filename is **display-only** and a **single click selects the row** — it never edits and no longer opens; instead the **chevron expands an inline preview** and the **bullet drills** to the node page. An **image** is the one exception (see below). |
| File node is a normal node | A file node behaves like any node, plus an inline preview: the **chevron expands an inline preview widget** under the row (the same widget the node page uses, started collapsed/peek), and any real **children** render below it. It can carry child notes, be moved, referenced, pinned, and opened on its node page — all for free. The row carries no inline text editor; a visually-hidden, focusable **keyboard anchor** (a lightweight element, not a read-only editor) gives the row full keyboard parity — arrow nav, Enter to add a sibling, Tab to indent, Backspace to remove the node. |
| File node row click & reference | A single click anywhere on the row selects it (it never drops a caret into the hidden anchor); the chevron toggles the inline preview and the bullet drills to the node page. A `reference` whose target is a file node still renders as a normal **reference row**, not with the file's icon-bullet / inline image. |
| File node preview | The preview shows in two places: **inline** under an expanded file row, and as the **node page** hero (drill the bullet). Both render the same `FilePreviewShell` — on the node page an outliner-ancestry breadcrumb + **read-only filename title**, then the rendered file under a single bottom-center floating pill, then the node's children outline. The pill's primary toggles **Expand/Collapse** (collapsed peek vs full vertical scroll) for a previewable file, or **Open with default app** for a non-previewable one; its `⋯` menu carries Show in Finder / Open with default app / Copy. The node-page title prefers the node display text, then the stored original filename, so old file nodes with blank text do not show `Untitled`. |
| Image render (inline) | An image is the one file kind that renders inline as the image **itself** (an image's content is its identity) instead of an icon-bullet name row: a bounded `<img>`, with no file-type icon and no filename in the row. Its `⋯` menu floats at the image's top-right, revealed on hover; clicking the image maximizes it (opens the file preview). |
| File node ⋯ menu | The non-image name row carries a hover `⋯` action menu whose first action is **Open in split** (opens the node page in a split pane), followed by the stored asset's system actions **Open**, **Reveal in Finder**, and **Copy file**; an image's `⋯` leads with **Maximize**. Inside a preview, the same system actions live on the floating pill's `⋯` menu. The row menu reveals on hover or keyboard focus and stays visible while open. |
| File node actions | The preview pill's `⋯` menu carries the stored asset's system actions — **Open with default app**, **Show in Finder**, **Copy file** — and for a non-previewable file **Open with default app** is the pill's primary. Open uses the OS default app after the main process revalidates the asset path and local-file policy. Reveal shows the stored asset copy. Copy puts the stored asset file on the clipboard. |
...
| Non-node source preview | A source with no node (agent payload, loose inline local-file ref, url) opens the same `file-preview` surface in its loose state: source/path breadcrumb, read-only filename/source title, and the shared preview with its floating pill, but no children outline. The pill's primary opens the source — **Open in browser** for a url, **Open with default app** for an on-disk source; its `⋯` menu offers **Show in Finder** for an on-disk source (local file / asset) and **add to outline** for ingestible kinds (below). |
| Add a non-node source to the outline | The loose preview's pill `⋯` menu offers "add to outline" for ingestible kinds (`local-file`, `agent-payload`; not `url`): it copies the source into the asset store, creates a file node under Today, and binds the same mounted preview surface to that node in place. After that it is an ingested node with outliner ancestry, a children outline, and file-node actions. |
