...
| File node preview (node page) | Opening the file (a card click, the `⋯` menu, or drilling the bullet) shows it as a node page with a **read-only filename title** and a preview "hero" — the rendered file with its meta + actions — above the node's children outline. The title prefers the node display text, then the stored original filename/source identity, so old file nodes with blank text do not show `Untitled`. This is the single full-preview surface (plus split-pane peek); there is no inline preview block in the outline. |
| Image render (inline) | An image is the one file kind that renders inline as the image **itself** (an image's content is its identity) instead of a file card: a bounded `<img>`, with no file-type icon and no filename in the row. Its `⋯` menu floats at the image's top-right, revealed on hover; clicking the image maximizes it (opens its node page). |
| File node ⋯ menu | Both presentations carry a `⋯` action menu whose first action is type-specific — **Maximize** for an image (opens the node page) and **Open in split** for other files (opens the node page in a split pane) — followed by the stored asset's system actions **Open**, **Reveal in Finder**, and **Copy file** (the same descriptor set the node-page hero shows). It reveals on hover or keyboard focus and stays visible while open. |
| File node actions | The node-page preview hero carries Open / Reveal / Copy alongside `type · size · pages/duration` meta. Open uses the OS default app after the main process revalidates the asset path and local-file policy. Reveal shows the stored asset copy. Copy puts the stored asset file on the clipboard. |
| Missing asset metadata | The node-page preview shows an unavailable message; the row stays an ordinary file row with a display-only filename and exposes no broken system actions. |
