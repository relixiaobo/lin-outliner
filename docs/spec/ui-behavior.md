...
single matching direct entry on the owner. Multiple matching entries are an
honest ambiguity: the write refuses with their entry ids and directs the caller
to address the intended entry by id or rename one field. When no direct entry
matches and Schema contains several active definitions with that label, the
owner's applied tag chains provide precedence. Resolution checks the direct tags
first, then each inheritance depth in specific-first order; the first layer with
exactly one unique matching `fieldDefId` wins. No reachable candidate or more
than one candidate at that layer preserves the duplicate-definition error.
...
| EPUB bilingual translation | A reflowable EPUB file-node page and dedicated reader place the shared `Languages` control in the stable header; compact inline outliner previews and `pre-paginated` books do not expose it. Asset and trusted-local packages use the existing validated internal stream with a 128 MiB compressed-package cap instead of inheriting the generic 20 MiB byte-read limit; streamless sources retain the bounded fallback. Target language, model, Translate / Show original, completion state, and `Option+A` / `Alt+A` match URL translation, while **Translate automatically** is a separate remembered EPUB opt-in that defaults off and never inherits webpage consent. Loaded sections contribute direct leaf-readable semantic blocks only; same-target blocks are skipped. EPUB uses the same complete-viewport loading, eight-block visible batches, sixteen-block prefetch batches, six-request shared pool, predictive three-to-eight-viewport window, local failure recovery, and visible-work preemption as URL pages; upward batches still send adjacent context in document order. Translation events wake the controller directly. Enabling translation expands lazy section mounting from 800px to the bounded eight-screen maximum, while unvisited sections beyond that window remain unloaded and unsent; remounted sections restore matching book-session cache. Loading, retry, stale-source cleanup, plain-text insertion, and Show original preserve the visible reading anchor, and real user scrolling always cancels delayed compensation. Target/model changes cancel and reset the old configuration; localized label changes update the live adapter without turning translation off. |
