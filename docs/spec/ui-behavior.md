...
| File node is a normal node | A file node behaves like any node: the chevron expands its **children** (not a preview), and a childless file node is a leaf. It can carry child notes, be moved, referenced, pinned, and opened in a split pane — all for free. The preview is not inline; it lives on the node page. |
| File node preview (node page) | Drilling the bullet opens the file as a node page whose body is the preview "hero" — the rendered file with its meta + actions — shown above the node's children outline. This is the single full-preview surface (plus split-pane peek); there is no inline preview block in the outline. |
| Image thumbnail (inline) | An image is the one file kind that shows inline: a bounded row-level thumbnail under the filename (part of the row, not a child block, so it never collides with the chevron's children). Clicking it opens the image's node page. Every other file kind is just an icon + filename row. |
