...
| Non-node source preview | A source with no node (agent payload, loose inline local-file ref, url) opens the same `file-preview` surface in its loose state: source/path breadcrumb, read-only filename/source title, and the shared preview, but no children outline. URL sources are previewable but not file-like: the breadcrumb/header shows the webpage favicon and title when the webview reports them, falling back to the link label or URL, and the body starts directly with the sandboxed URL preview webview. URL previews are single-layer like audio/video: no document preview outer frame, duplicate large file heading, loading overlay, Expand/Collapse primary, resize handle, or bottom floating action bar over the page. The webview surface itself carries the shared outer edge treatment (`--file-preview-frame-radius` plus `--inset-hairline`) and fills the available pane height. A neutral `Languages` icon before the header `⋯` opens a compact popover with target-language and model selectors, an automatic-translation switch, and a filled full-width Translate page / Show original command with the matching show/hide icon. At least one completed translation overlays a check on the icon, while an enabled page with no eligible untranslated blocks stays idle without claiming completion. The target follows the UI locale until the user explicitly selects one, after which that selection is remembered across pages and launches. The model defaults to dynamic `Follow Agent`; an explicit enabled/runnable model is remembered by qualified provider id, remains visibly unavailable if its configuration later disappears, and never silently falls back. Changing model cancels work, clears page-local results, and retranslates the current viewport. Automatic translation defaults off and is remembered globally. When enabled, it immediately evaluates the current page and translates only if a valid top-level `<html lang>` differs from the target; missing or invalid metadata remains manual. Turning auto off leaves visible translations intact, while manual Show original suppresses auto only until the next top-level navigation. `Option+A` on macOS and `Alt+A` elsewhere toggles the active URL preview, including while its webview has focus, without changing auto. On enable, visible readable blocks run first, followed by roughly two viewports ahead and half a viewport behind; the whole page is never submitted eagerly, and any block whose nearest declared `lang` matches the target is skipped without loading or a provider call. Each submitted source gets a fixed-size inline loader that does not scale with the webpage's typography; success replaces it with inert translated text after the source, while failure changes it into a keyboard-accessible error control. Activating that control retries only its paragraph, and normal scheduling stays paused while failed blocks await explicit retry, including after a missing-model error. Original and completed translations stay readable and the existing dismissible error toast announces the failure. Hiding/showing cached results and inserting status or translation nodes preserve the first visible source block's viewport offset. Closing, navigating, reloading, or changing target/model cancels work; navigation/reload re-evaluates auto after DOM readiness. URL previews expose **Open in browser** from the header `⋯`. Other previewable and non-previewable loose sources use the same bottom-center action location; non-previewable sources render it as the metadata card footer to avoid overlaying metadata. The primary action opens an on-disk source with its default app; the `⋯` menu offers **Show in Finder** for an on-disk source (local file / asset) and **add to outline** for ingestible kinds (below). Stored assets expose `asset://<id>` and trusted local files expose `preview-local://<token>` through the same `streamUrl` field, so image/audio/video renderers do not branch on whether the source came from the outliner or an agent working file. |
