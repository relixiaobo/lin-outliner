...
| Agent transcript file preview | Live agent transcript file chips are local working-file pointers. A click opens a file-only reader inside the agent dock, covering the message stream rather than opening a workspace split pane. The reader uses the same file preview content shell as workspace file previews, starts in full reader mode, has no Expand/Collapse or resize handle, and shows only a compact header with back, filename, and `⋯` actions. The `⋯` menu keeps the system actions: **Open with default app**, **Show in Finder**, and **Add to Today** for regular files. Closing the reader returns to the transcript. |
