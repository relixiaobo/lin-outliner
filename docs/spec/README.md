...
| `agent-tool-permissions.md` | The delegated-operator capability model: persistent folder access, private Tenon control state, process containment, user blocks, scoped tool catalogs, and capability events. | Changing how tool calls execute, acquire a folder, become unavailable, or are recorded in the capability store. |
