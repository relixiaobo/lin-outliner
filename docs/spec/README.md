...
| `agent-tool-permissions.md` | The runtime capability-first permission policy: persistent folder access, direct hard/user/restricted blocks, process containment, the permission store, and events. | Changing how tool calls execute, request a folder, are blocked, or are recorded in the permission store. |
