...
| `agent-pi-mono-implementation.md` | pi-mono runtime boundary and Electron integration. | Changing provider loop, runtime execution, capabilities, keys, streaming, or pi-mono adapter behavior. |
| `agent-skills.md` | Skill discovery, loading, slash invocation, compaction restore, runtime-only memory Dream, and isolated-skill tool catalogs. | Changing agent skills, `/compact`, runtime-only memory Dream, isolated skill tool access, or skill settings. |
...
| `agent-tool-permissions.md` | The delegated-operator access model: default Full Access, optional Restricted folder/process containment, mode-specific Tenon control-state authority, user blocks, scoped tool catalogs, and capability events. | Changing how tool calls execute, select filesystem authority, acquire a folder, become unavailable, or are recorded in the capability store. |
