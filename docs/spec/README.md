...
| `agent-tool-permissions.md` | The delegated-operator access model: host-account Full Access, explicit user blocks, scoped tool catalogs, shared-resource concurrency boundaries, native OS/provider authority, and capability audit events. | Changing how tool calls execute, access host files/processes, become unavailable, or are recorded in the capability store. |
