...
| --- | --- | --- |
| Shell and rails | `App.tsx`, `WindowChrome.tsx`, `Sidebar.tsx`, `ThreadDock.tsx`, `useWorkspaceLayout.ts` | [Surfaces → Shell](./design-system/surfaces.md#shell) owns window chrome, floating rails, shell layout, and collapse controls. |
...
| Outliner panel | `NodePanel.tsx`, `OutlinerFlatView.tsx`, `OutlinerItem.tsx`, `styles/outliner.css` | [Surfaces → Outliner](./design-system/surfaces.md#outliner) owns title, breadcrumb, tags, rows, fields, references, and triggers. |
...
| Editor and commands | `RichTextEditor.tsx`, `CodeBlockSurface.tsx`, `FloatingEditorToolbar.tsx`, `nodeLineTrigger.ts` | [Components → Overlays](./design-system/components.md#overlays) owns overlay semantics; [Surfaces → Menus, Popovers, And Dialogs](./design-system/surfaces.md#menus-popovers-and-dialogs) owns command/menu surfaces. |
...
| Agent dock | `ThreadDock.tsx`, `ThreadList.tsx`, `ThreadView.tsx` | [Surfaces → Agent](./design-system/surfaces.md#agent) owns the persistent Thread dock, history scroll, actions, and composer. |
| Agent Items | `ThreadItemView.tsx`, `ThreadGoalView.tsx`, `UserInputRequest.tsx` | [Components → Activity And Disclosure Rows](./design-system/components.md#activity-and-disclosure-rows) owns dense execution rows; [Surfaces → Agent](./design-system/surfaces.md#agent) owns Item and Goal surfaces; [Foundations → Color & Appearance](./design-system/foundations.md#color--appearance) owns gallery media-HUD contrast. |
| Source previews and inline files | `NodeSourcesSection.tsx`, `FilePreviewBody.tsx`, `FilePreviewPanel.tsx`, `FilePreviewPill.tsx`, `InlineFileReference.tsx`, `InlineFilePreviewLayer.tsx`, `inlineFileIcon.ts` | [Components → File Preview Frame And HUD](./design-system/components.md#file-preview-frame-and-hud) owns preview chrome; [Patterns → File Preview Flow](./design-system/patterns.md#file-preview-flow) owns inline preview behavior. |
| Agent settings | `SettingsWindow.tsx`, `configuration/ManagerWindow.tsx`, `configuration/PreferenceRow.tsx`, `AboutContent.tsx`, `ModelsList.tsx`, `AccessRules.tsx`, `SkillLibrary.tsx`, `ShortcutManager.tsx`, `ManagedSkillsSettings.tsx`, `MemoryManager.tsx`, `DataMaintenanceRow.tsx`, `SettingsInsetList.tsx`, `SettingsRowMenu.tsx`, `ProviderConfigWindow.tsx` / `ProviderConfigForm.tsx` / `ProviderOAuthForm.tsx`, `providerStatus.ts`, `settingsProviderModel.ts`, `providerCatalog.tsx`, `styles/settings-*.css` | [Surfaces → Settings Window](./design-system/surfaces.md#settings-window) owns single-window Settings navigation, independently owned domain panes, inset grouped content, row menus, and owned credential windows. |
...
| Global launcher uses vibrant system glass. | The separate system launcher window only; in-app dialogs remain opaque elevated surfaces. | [foundations.md → Materials & Liquid Glass](./design-system/foundations.md#materials--liquid-glass) | `docs/spec/launcher.md` + launcher visual verification for transparency/reduced-transparency changes. |
| Media HUD controls own fixed contrast tokens. | File-preview actions and compact image-gallery overflow controls over arbitrary document/image pixels. | [foundations.md → Color & Appearance](./design-system/foundations.md#color--appearance) | `--media-hud-*` token presence in `src/renderer/styles/tokens.css` + focused file-preview/gallery visual checks over mixed pixels. |
| Preview action HUD may use backdrop material. | Bottom-center file-preview action controls only; compact media-HUD badges do not inherit blur or preview elevation. | [components.md → File Preview Frame And HUD](./design-system/components.md#file-preview-frame-and-hud) | `previewHudBackdropSelectors` in `tests/e2e/typography-tokens.spec.ts` + focused file-preview visual checks. |
...
| Composer attachment carousel hides its redundant scrollbar. | Agent composer attachment tray only; touchpad scrolling, edge buttons, and keyboard navigation must all remain available. | [surfaces.md → Agent](./design-system/surfaces.md#agent) | `tests/e2e/typography-tokens.spec.ts` hidden-scrollbar guard + focused Agent attachment E2E. |
...
| Remote video captions use fixed contrast colors. | Bilingual caption overlays injected by the isolated URL-preview guest only; app chrome remains tokenized. | [foundations.md → Color & Appearance](./design-system/foundations.md#color--appearance) | `scripts/design-system-metrics.ts` named functional-colour exceptions + `tests/renderer/urlPageTranslationGuest.test.ts` caption CSS regression coverage. |
