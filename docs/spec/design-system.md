...
- Sidebar rows use one quiet navigation grammar: `28px` row height,
  `6px` radius, and `16px` icon slots. Primary sidebar entries use the shared
  `--control-hover` fill while hovered; the workspace root outline uses text
  darkening only so it reads as a quiet tree. Current pages do not keep a
  persistent selected fill. Workspace root is a clickable row with a compact
  avatar; root children keep the chevron before the icon/text slot and use
