...
| --- | --- | --- |
| `CheckboxMark` | `src/renderer/ui/primitives/CheckboxMark.tsx`, `CheckboxControl.tsx`, `DoneCheckbox.tsx`, field value checkbox rows | Shared primitive; owns three-state visual mark only, not row, form, or settings behavior. |
| `CheckboxControl` | `src/renderer/ui/primitives/CheckboxControl.tsx`, `AgentSettingsDialog.tsx` | Extracted labeled native checkbox wrapper; keeps settings checkbox semantics and the shared `CheckboxMark` together. |
...
| `ButtonControl` | `src/renderer/ui/primitives/ButtonControl.tsx`, app shell, sidebar, workspace tabs, command palette, outliner rows, tags, and agent surfaces | Extracted for button semantics, default button type, and focus refs; surface-specific visual variants remain class-owned. |
...
| `SwitchControl` | `src/renderer/ui/primitives/SwitchControl.tsx`, `DefinitionConfigPanel.tsx`, field boolean rows | Extracted semantic switch wrapper for `role=switch`, `aria-checked`, checked toggle, and focus refs; visual treatment remains surface-owned. |
| `SelectControl` | `src/renderer/ui/primitives/SelectControl.tsx`, `DefinitionConfigPanel.tsx`, `AgentSettingsDialog.tsx`, `ViewToolbar.tsx` | Extracted native select wrapper for accessible label, ref forwarding, and prop forwarding; options and value coercion remain caller-owned. |
| `TextInputControl` | `src/renderer/ui/primitives/TextInputControl.tsx`, definition controls, agent settings/session controls, command palette, context search, option/tag pickers, field rows | Extracted native input wrapper for accessible label, type, ref forwarding, and prop forwarding; draft/commit behavior remains caller-owned. |
| `NumberInputControl` | `src/renderer/ui/primitives/NumberInputControl.tsx`, `DefinitionConfigControls.tsx` | Extracted native number input wrapper for accessible label, ref forwarding, and prop forwarding; parsing and empty-value semantics remain definition-owned. |
