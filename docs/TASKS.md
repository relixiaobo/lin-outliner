...
The single live board across all clones — who's doing what now, the active-plan
index, and what's shipped. **Owned by the main agent** (`lin-outliner/`); the
...
body) before building — they do not edit this file. `docs/plans/<topic>.md`
holds each item's detailed design (terminal plans live in
...
The active-plan index (folds the former `docs/plans/README.md`). Each row links
its plan file; terminal plans live in `docs/plans/archive/`. This is the *what
plans exist* catalog — **Backlog** below is the *what to do next* prioritized
cut and may include non-plan items.

| Plan | Status | Pri | Summary |
| --- | --- | --- | --- |
| [`asset-subsystem`](plans/asset-subsystem.md) | in-progress | P0 | Local content-addressed asset store + `asset://` protocol. Blocks image/attachment/embed work. |
| [`lazy-like-global-launcher`](plans/lazy-like-global-launcher.md) | draft | P0 | Lazy-like global launcher: product target, external refs, UX, architecture, context capture, permissions/provider boundaries, implementation plan, acceptance criteria. |
| [`agent-past-chats`](plans/agent-past-chats.md) | in-progress | P1 | Single `past_chats` tool (recent + search + read) over the event store, user-message/search indexes, and visible transcript filtering. |
| [`agent-self-modification`](plans/agent-self-modification.md) | draft | P1 | Controlled self-maintenance: runtime status, cc-2.1-style config reads/writes, hooks, skill creation/curation, recovery, audit, rollback. |
| [`design-system-rollout`](plans/design-system-rollout.md) | in-progress | P1 | Stage `design-system.md` into code: two-theme alpha-on-ink tokens + dark mode, Electron material/vibrancy + nativeTheme toggle, neutral-functional components, floating-rails shell. |
| [`native-feel-remediation`](plans/native-feel-remediation.md) | in-progress | P1 | Make the Electron app feel native (macOS + Windows). Stages 1–5 shipped; only packaging/smoke (stage 6) remains. |
| [`image-rendering`](plans/image-rendering.md) | in-progress | P1 | Render `image` nodes inline via the asset subsystem; paste/drag/clipboard ingestion. |
| [`file-attachments`](plans/file-attachments.md) | draft | P1 | New `attachment` node type for arbitrary local files (PDF/audio/video/generic). Beyond nodex parity. |
| [`agent-composer-attachment-path-model`](plans/agent-composer-attachment-path-model.md) | draft | P1 | Path-first agent-composer attachments: only images stay inline; everything else is a path reference; no-path inputs staged via `lin:stage-attachment`. |
| [`agent-oauth-providers`](plans/agent-oauth-providers.md) | draft | P2 | OAuth sign-in (Anthropic Pro/Max, GitHub Copilot, OpenAI Codex) + managed credentials (Bedrock, Vertex) for provider settings. |
| [`agent-tool-permissions-hardening`](plans/agent-tool-permissions-hardening.md) | draft | P2 | Non-blocking follow-ups after #60: sessionApproved ordering, config re-validation, extended exfil redline, event-vocab collapse, denied-reason alignment. |
| [`agent-scheduled-routines`](plans/agent-scheduled-routines.md) | draft | P2 | Proactive agent `command` NodeType — NL brief; user-only `date` (anchor + optional RRULE) arms an offline anacron scheduler. |
| [`outliner-local-file-references`](plans/outliner-local-file-references.md) | draft | P2 | First-class `@file`/folder references in the outliner — shared data model, marker parser, editor behavior, preview/open, agent context normalization. |
| [`floating-toolbar-polish`](plans/floating-toolbar-polish.md) | draft | P3 | Heading-mark toggle + `#` selection-extract in the floating editor toolbar. |
| [`embed-strategy`](plans/embed-strategy.md) | draft | P3 | Decide live iframe vs cached-metadata embeds, or remove the embed schema fields. |
| [`nodex-parity-decisions`](plans/nodex-parity-decisions.md) | meta | — | Catalog of nodex features we explicitly will not port and why. |
