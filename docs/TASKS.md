...
taxonomy are now Tenon-owned and `pi-agent-core` is deleted; see *Agent
capabilities*. #444
...
- **tag-selector-active-tag-index**
  ...
  fast-track) — active tag definitions, normalized labels, hexadecimal-color
  penalties, exact-label membership, and empty-query ordering are cached once
  per renderer projection snapshot. Repeated selector opens and query changes
  reuse that snapshot; empty-query menus skip already-applied tags and stop at
  the visible limit without rescanning or reranking the full tag set.
  **Gate (main):** review found no reportable issues on final head `f4472ec2`.
  Verified with typecheck, `docs:check`, diff check, 74 focused renderer tests,
  full `test:renderer` (962 pass), and a 60,000-call randomized old-versus-new
  differential check.

- **field-name-reuse-candidate-index**
  (`codex/field-name-reuse-candidate-index`, PR #426, codex, merged 2026-07-21,
  fast-track) — field-name reuse now caches active definitions and Trash ancestry
  per renderer projection snapshot instead of rescanning the complete document on
  every focused keystroke. Typed queries keep the existing complete prefix-first
  result contract and localized display order.
  **Gate (main):** the first review found that localized index ordering could hide
  real ASCII prefix candidates and that a 24-result bound changed user-visible
  behavior without a plan. Codex switched the search index to lowercase code-unit
  ordering and restored complete results on final head `fbea1917`. Verified with
  typecheck, `docs:check`, diff check, 87 focused renderer tests, full
  `test:renderer` (961 pass), and focused light/dark E2E (2 pass).
...
- **panel-date-navigation-index**
  ...
  fast-track) — renderer document state now maintains day-note tag membership
  and per-date direct-child counts from projection deltas. The panel calendar
  reads only its visible date window instead of rescanning the full document,
  while fallback Day tags, renames, duplicate dates, and sparse updates preserve
  existing behavior.
  **Gate (main):** the first review found that ordinary incremental updates still
  cloned complete backing maps; the second found that every tag-member set
  eagerly allocated 1,024 empty buckets. Codex replaced those paths with sparse
  occupied-bucket maps and native sets below 64 members on final head `05f6baa`.
  Verified with typecheck, `docs:check`, diff check, full `test:renderer` (953
  pass), focused light/dark date-navigation E2E (3 pass), and a 5,000-operation
  incremental-versus-rebuild differential check across the set threshold.
...
- **renderer-delta-reducer-surface**
  ...
  fast-track) — renderer projection delta folding now keeps projection `byId`
  and per-node render revisions in bucketed copy-on-write sparse maps, and
  exposes delta `projection.nodes` through a lazy array-shaped compatibility
  view. Ordinary deltas patch only changed/removed ids, preserve unchanged node
  object identity, and avoid materializing the previous map or full node array
  on the hot reducer path.
  **Gate (main):** review found no reportable issues on final head `be8f174`.
  Verified with typecheck, `docs:check`, diff check, full `test:renderer` (936
  pass), and focused sparse projection / reducer / real-Core delta integration
  tests.
...
- **main-document-read-model**
  ...
  fast-track) — `DocumentService` now owns a main-side `DocumentReadModel` that
  seeds from a full projection once, applies `ProjectionUpdate` deltas in place,
  and stays fresh even when no renderer projection listener is registered. Agent
  `node_read` and `node_search` prefer that maintained `ProjectionIndex`, while
  `node_edit replace_outline` consumes transaction-local sparse projection
  facts from Core/DocumentService so multi-command edits can update their
  in-flight view without rebuilding full projections between subcommands.
  **Gate (main):** review found no reportable issues on final head `bfa4e203`.
  Verified with typecheck, `docs:check`, diff check, focused read-model /
  DocumentService / Agent node-tool suites, full `test:core` (1676 pass), and
  `LIN_VERIFY_CACHE=1` focused cache verification.
