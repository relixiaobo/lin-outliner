...
| S3 | hairline dividers throughout | `--separator` = 0.10 ink | `tokens.css:72`; consumers in `agent-tool-rows.css`, `agent-run-detail.css`, `code.css`, `agent-debug.css`, `settings-agents.css` | 0.10 white on dark is very faint; where it divides two near-equal dark surfaces it may vanish (no opaque fallback outside `prefers-contrast`). | If invisible where it matters: lift `--separator` in the dark block, e.g. 0.10 → **0.14–0.16**. One line. |
