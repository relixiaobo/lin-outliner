...
| **Retrieval** | three modes: chronic activation (resident briefing) · deliberate cued retrieval (`recall` + source access) · automatic association (deferred) | §8 assembly + the single `recall` tool; chronic activation renders a schema overview + strength-selected facts; deliberate `recall` ranks with a rebuildable hybrid ranker (BM25-class lexical + retrieval strength + query-time `sources[]` co-citation association; no embeddings in PR-4); the zoom ladder (schema → fact → episode gist → raw span, D-6) is the provenance axis |
