...
A catalog of nodex features that Tenon **will not** port, plus the reason.
...
current code (for that see
[`outliner-parity-matrix.md`](../../spec/outliner-parity-matrix.md)).

## Already in Tenon (no action needed)

The first comparison pass missed these. Recorded here so we don't accidentally
re-plan them:

- **Zoom-in by bullet click** — `OutlinerItem.tsx` (`onDrillDown`).
- **Breadcrumb with collapsed ancestors** — `NodePanel.tsx` plus the
...
| --- | --- |
| Multi-panel side-by-side (nodex `Panel` + `NavigationEvent`) | Tenon's workspace tab + WorkspaceCanvas covers the same ergonomic with a different shape. Adding side-by-side later is a tab-system enhancement, not an outliner change. |
...
| Inline chat / agent message panel inside the document | Tenon runs agents through a separate dock (`AgentDock.tsx`) with a real provider runtime. Bringing chat back into the document blurs the outliner/agent boundary we deliberately drew. |
...
| Tana import (nodex `services/tana-import.ts`) | Not a nodex-parity port. The chosen direction is the built-in `outline` Skill's import workflow over generic public ChangeSets, with Tana as the first deterministic route; see [`outliner-runtime-cli.md`](../archive/outliner-runtime-cli.md). |
| `Filter` rules as child nodes of `viewDef` | Tenon keeps **view** config typed (filter fields with typed enums), now on the dedicated `FilterRuleNode` union variant rather than the god-record (PR #18, A-full). Note: **definition** config (tag/field knobs) *did* move to child nodes as of PR #18 (`defConfig` subtrees) — config-as-nodes reverses this decision for definition config only; view config stays typed. |
| Chat panel as workspace panel kind (`isChatPanel`, `CHAT_PANEL_PREFIX`) | Same reason as "inline chat". |
| Wider trigger characters (`>` for create-field, `!` actions, `[[` wikilink) | Only `>`/`#`/`@`/`/` are in Tenon today. Open to per-trigger additions if a real workflow needs them, but no blanket parity. |
| Sync (`stores/sync-store.ts`) | Tenon is single-device today. Sync is a separate large plan, not nodex parity. |
...
| `AutoCollectSection.tsx` / `AutoInitGroup.tsx` as distinct field config panels | Their Tenon equivalents are folded into `DefinitionConfigPanel.tsx`. If that panel grows past ~600 lines, split along these lines. |
| Field validation panel (`field-validation.tsx`) | When Tenon gains stricter field validation, follow nodex's separation. |

## Deliberate supersets (Tenon > nodex)

Things Tenon has that nodex does not, kept here so the asymmetry is visible:
...
- Generic file attachments (see
  [`file-attachments.md`](../archive/file-attachments.md)) —
