...
| Tana import (nodex `services/tana-import.ts`) | Not a nodex-parity port. The chosen direction is the built-in `outline-import` workflow over generic public ChangeSets, with Tana as the first deterministic route; see [`outliner-runtime-cli.md`](../outliner-runtime-cli.md). |
