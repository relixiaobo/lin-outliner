...
| [`reference-field-type.md`](reference-field-type.md) | done | P2 | One reference-node model for node-reference field values: read-only computed fields (`References`/`Owner`/`Day`) project synthetic read-only reference rows (full reference behavior — double-click edits target, expandable — but no add/delete), and a new editable `reference` field type (`@`-pick any node, stored reference child). Deletes the bespoke `.field-value-link`. |
