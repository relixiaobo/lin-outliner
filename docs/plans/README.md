...
| [`native-settings-redesign.md`](native-settings-redesign.md) | draft | P2 | Evolve the agent Settings surface toward the macOS System Settings *interaction* idiom (inset grouped lists, master-detail, credential sheet, on-row status) in our own tokens/B-rules. **Blocked on PR-D** (settings-window invocation); coordinate with `agent-oauth-providers.md` (shared credential sheet) and bundle a perf pass. |
