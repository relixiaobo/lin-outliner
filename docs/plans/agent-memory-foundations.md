...
| **Metamemory** | knowing what one knows (feeling-of-knowing) before attempting retrieval | Nelson & Narens | the schema/overview layer (PR-5): the briefing's breadth axis tells the model what it knows before it decides to dig |
