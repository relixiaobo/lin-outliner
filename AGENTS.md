...
| Dev run (per clone) | `bun run dev:<main\|cc\|cc-2\|codex\|anti>` — isolates `userData` |
...
Five clones run side-by-side under `~/Coding/`, sharing one GitHub origin
...
- **Dev agents** (`cc`, `cc-2`, `codex`, `anti` — all equal) — **draft** the plan
  (what + how) AND build it. `codex` is a different model (Codex/GPT); the rest
  are Claude Code. No agent has a special role beyond its clone.
