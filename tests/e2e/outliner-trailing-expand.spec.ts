...
test.describe('outliner trailing input and expansion parity', () => {
  ...
  test('collapsing a long flat row keeps the clicked chevron visually anchored', async ({ page }) => {
    const anchorParentId = 'anchor-parent';
    ...
    await expect(row(page, 'anchor-child-0')).toBeVisible();
    await expect(page.locator('.outliner-flat')).toBeVisible();
    ...
    await expect(row(page, 'anchor-child-0')).toHaveCount(0);
