...
test.describe('outliner navigation and page title parity', () => {
  ...
  test('clicking a node bullet drills into that node page', async ({ page }) => {
    ...
    await expect(page.locator('.panel-title-editor')).toContainText('Alpha');
    await expect(page.getByRole('navigation', { name: 'Breadcrumb' })).toContainText('Alpha');
  });
  ...
  test('node page title is editable and writes back to the same node', async ({ page }) => {
    ...
    await expect(page.locator('.panel-title-editor')).toContainText('Alpha renamed');
