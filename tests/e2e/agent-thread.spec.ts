...
test.describe('canonical agent Thread surface', () => {
  ...
  test('says a command failed without inventing an exit code it never reported', async ({ page }) => {
    ...
    // A timeout produced neither a code nor output, so the detail has nothing to
    // add: the row's own `failed` segment is the whole statement, and no
    // plausible-looking code is borrowed to fill the gap.
