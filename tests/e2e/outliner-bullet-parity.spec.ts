  ...
  test('field label keeps first-line alignment when the value contains multiple rows', async ({ page }) => {
    ...
    const metrics = await priorityRow.evaluate((element) => {
      ...
      const firstValueLeading = element
        .querySelector('.field-value-outliner > .outliner-flat-flow > .outliner-flat-flow-row:first-of-type > .row-wrap > .row > .row-leading')
        ?.getBoundingClientRect();
      const firstValueRow = element.querySelector('.field-value-outliner > .outliner-flat-flow > .outliner-flat-flow-row:first-of-type > .row-wrap > .row');
