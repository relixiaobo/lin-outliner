...
describe('agent tool call block', () => {
  ...
  test('summarizes canonical tools with readable labels instead of raw tool ids', () => {
    const cases: Array<{
      ...
      { name: 'issue_search', args: { query: 'daily report' }, pending: 'Searching issues "daily report"', done: 'Searched issues "daily report"' },
      { name: 'issue_read', args: { issueId: 'issue-1' }, pending: 'Reading issue "issue-1"', done: 'Read issue "issue-1"' },
