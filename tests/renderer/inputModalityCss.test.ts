...
describe('input modality CSS guards', () => {
  ...
  test('keeps flat media previews visible and non-blocking while chrome is hidden', () => {
    ...
    expect(filePreviewCss).toMatch(/\.file-preview-media-player\s*\{[^}]*border-radius:\s*var\(--file-preview-page-radius\);/s);
    ...
    expect(filePreviewCss).toMatch(/\.file-node-preview--media\s*\{[^}]*overflow:\s*hidden;/s);
    expect(filePreviewCss).toMatch(/\.file-node-preview\.file-node-preview--media\s*\{[^}]*padding-bottom:\s*var\(--file-preview-frame-padding-block\);/s);
    expect(filePreviewCss).not.toMatch(/\.file-node-preview--media\s*\{[^}]*border-radius:\s*var\(--radius-md\);/s);
    expect(filePreviewCss).not.toMatch(/\.file-node-preview--media\s*\{[^}]*box-shadow:\s*none;/s);
