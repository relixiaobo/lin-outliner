...
describe('WorkingText', () => {
  ...
  test('WorkingText confines its animation to a paint-contained glyph layer', () => {
    expect(workingTextCss).toMatch(
      /\.working-text-sweep-copy \{[^}]*background-position:\s*-100% 0;[^}]*background-size:\s*50% 100%;[^}]*-webkit-background-clip:\s*text;[^}]*background-clip:\s*text;[^}]*-webkit-text-fill-color:\s*transparent;/s,
    );
