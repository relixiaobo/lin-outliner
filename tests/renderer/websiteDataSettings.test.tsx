...
import type { ClearUrlPreviewDataResult } from '../../src/core/urlPreviewSession';
import { WebsiteDataSettingsGroup } from '../../src/renderer/ui/agent/WebsiteDataSettingsGroup';
...
describe('WebsiteDataSettingsGroup', () => {
  test('keeps a stable busy row and reports a completed clear', async () => {
    let resolveClear: ((result: ClearUrlPreviewDataResult) => void) | null = null;
    const clearResult = new Promise<ClearUrlPreviewDataResult>((resolve) => {
    ...
    expect(notices).toEqual([null, 'Website data cleared.']);
  });
  ...
  test.each([
    [{ status: 'failed', error: 'unavailable' }, 'Website data is unavailable in this window.'],
    [{ status: 'failed', error: 'clear-failed' }, 'Could not clear website data.'],
...
function renderComponent(
  clearUrlPreviewData: () => Promise<ClearUrlPreviewDataResult>,
