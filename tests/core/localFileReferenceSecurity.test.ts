...
describe('local file reference security', () => {
  ...
  test('rejects relative paths even when a same-named file exists under an allowed root', async () => {
    const root = await mkdtempRoot('lin-local-file-root-');
    await writeFile(path.join(root, 'notes.md'), 'note');
    await expect(resolveTrustedLocalFileReference('notes.md', [root])).resolves.toBeNull();
    await expect(resolveTrustedLocalFileReference('../secret.png', [root])).resolves.toBeNull();
  });
