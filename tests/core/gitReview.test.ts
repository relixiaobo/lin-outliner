...
test('raw reviewed untracked bytes are not replaced by a repository clean filter', async () => {
  ...
  const result = await commitReview(cwd, await captureReview(cwd, ['new.txt']), ['new.txt'], 'Raw reviewed content');
  expect(result.outcome).toBe('succeeded'); expect(git(cwd, 'show', 'HEAD:new.txt')).toBe('reviewed bytes');
});
