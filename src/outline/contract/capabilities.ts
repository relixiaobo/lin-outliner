...
export const OUTLINE_COMMAND_FAMILIES = Object.freeze([
  ...
  { name: 'source', summary: 'Add, replace, reorder, remove, or clear Node Sources.' },
...
const FIXED_COMMAND_HELP = Object.freeze({
  ...
  'view inspect': fixedHelp({ usage: 'view inspect TARGET', summary: 'Inspect one persisted view without returning item payloads.', behavior: 'read-only', idempotent: true, positionals: ['TARGET is one exact owner ID, typed ID, or semantic alias.'], options: [], selectors: selectorSyntax, cardinality: 'The owner target resolves exactly one live Node.', input: 'Accepts one positional exact target.', output: 'Returns one compact outline.view-summary with view mode, exact direct-item count, and ordered display metadata.', defaults: noDefaults, destructive: false, examples: ['outline view inspect node:project', 'outline --human view inspect @today'] }),
...
const FIXED_CAPABILITIES = [
  ...
  capability({ name: 'view inspect', kind: 'read', runtimeRequired: true, streaming: false, destructive: false, auditCategory: 'read.view', summary: 'Inspect one persisted view without returning item payloads.', requestSchema: Type.Object({ target: OneTargetRefSchema }, closed), resultSchema: ViewSummarySchema, coverage: [] }),
...
const PORCELAIN_COMMANDS = [
  ...
  ['source add', 'source.add', ['add_source']],
  ['source replace', 'source.replace', ['replace_source']],
  ['source reorder', 'source.reorder', ['reorder_source']],
  ['source remove', 'source.remove', ['remove_source']],
  ['source clear', 'source.clear', ['clear_sources']],
