...
const FIXED_COMMAND_HELP = Object.freeze({
  ...
  example: fixedHelp({ usage: 'example COMMAND [VARIANT]', summary: 'Print one validated executable recipe for a structured CLI workflow.', behavior: 'metadata', idempotent: true, positionals: ['COMMAND may contain multiple words. VARIANT is optional when that command has exactly one recipe.'], options: [], selectors: 'No selectors.', cardinality: 'Not applicable.', input: 'No input. The recipe contains literal stdin when required.', output: 'Returns bounded Command, Stdin, Result, and Review sections.', defaults: ['A command with exactly one recipe infers that variant.'], destructive: false, examples: ['outline example create collection', 'outline example search create', 'outline --json example transact dependent-change'] }),
  ...
  history: fixedHelp({ usage: 'history [OPERATION_ID] [FILTER OPTIONS]', summary: 'Read paginated durable Operation history.', behavior: 'read-only', idempotent: true, positionals: ['Optional OPERATION_ID selects one exact Operation; --operation is the equivalent explicit form.'], options: [option('limit', 'N', 'Bound returned Operations.', { default: '100' }), option('cursor', 'CURSOR', 'Continue one history page.'), option('operation', 'ID', 'Select one Operation ID.'), option('idempotency-key', 'KEY', 'Select by idempotency key.'), option('node', 'ID', 'Select Operations affecting one Node.'), option('origin', 'ORIGIN', 'Filter by operation origin.'), option('thread', 'ID', 'Filter by Agent Thread.'), option('turn', 'ID', 'Filter by Agent Turn.'), option('item', 'ID', 'Filter by Agent Item.')], selectors: 'History filters use stable IDs, not display-text selection.', cardinality: 'Returns at most --limit Operations and an optional cursor.', input: 'Use the positional Operation ID for an exact lookup or options for other filters.', output: 'Returns Operation summaries with recovery state and an optional cursor.', defaults: ['Limit defaults to the Runtime page default.'], destructive: false, examples: ['outline history operation:example', 'outline history --node node:project --limit 20', 'outline history --idempotency-key import:2026-08-24'] }),
