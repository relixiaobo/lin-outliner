...
const FIXED_COMMAND_HELP = Object.freeze({
  ...
  revert: fixedHelp({ usage: 'revert OPERATION_ID', summary: 'Guard and exactly revert one retained Operation.', behavior: 'recovery mutation', idempotent: false, positionals: ['OPERATION_ID is the visible ID returned by the original mutation.'], options: [], selectors: 'The Operation fixes the affected target set.', cardinality: 'Reverts exactly one retained Operation as one new Operation.', input: 'Accepts one Operation ID.', output: 'Returns the revert Operation with affected count and recovery state; conflicts do not write.', defaults: noDefaults, destructive: false, examples: ['outline revert operation:example', 'outline --json revert operation:example'] }),
  undo: fixedHelp({ usage: 'undo', summary: 'Revert the latest applicable Operation.', behavior: 'recovery mutation', idempotent: false, positionals: [], options: [], selectors: 'The latest applicable Operation fixes the target set.', cardinality: 'Reverts at most one Operation.', input: 'No input.', output: 'Returns one revert Operation.', defaults: noDefaults, destructive: false, examples: ['outline undo', 'outline --json undo'] }),
  redo: fixedHelp({ usage: 'redo', summary: 'Revert the latest applicable revert Operation.', behavior: 'recovery mutation', idempotent: false, positionals: [], options: [], selectors: 'The latest applicable revert fixes the target set.', cardinality: 'Reapplies at most one Operation.', input: 'No input.', output: 'Returns one redo Operation.', defaults: noDefaults, destructive: false, examples: ['outline redo', 'outline --json redo'] }),
