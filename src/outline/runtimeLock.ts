  ...
  static async acquire(paths: OutlineRuntimePaths, owner: RuntimeLockOwner): Promise<OutlineRuntimeLock | null> {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        const claimed = await stat(paths.lockPath);
        const lock = new OutlineRuntimeLock(paths.lockPath, owner);
        try {
          const handle = await open(path.join(paths.lockPath, 'owner.json'), 'wx', 0o600);
          try { await handle.writeFile(`${JSON.stringify(owner)}\n`); await handle.sync(); }
          finally { await handle.close(); }
          await syncDirectory(paths.lockPath);
          const active = await stat(paths.lockPath);
          if (active.dev !== claimed.dev || active.ino !== claimed.ino) throw new Error('Runtime writer claim changed during acquisition');
          return lock;
        } catch (error) {
          await lock.release();
          throw error;
...
async function activeLockOwner(lockPath: string): Promise<RuntimeLockOwner | null> {
  ...
  if (owner) return processIsAlive(owner.pid) ? owner : null;
