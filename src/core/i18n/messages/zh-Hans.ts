...
export const zhHans: PartialMessages = {
  ...
  settings: {
    ...
    security: {
      ...
      noBlocks: '暂无明确阻断规则。',
      boundariesGroup: '文件夹访问',
      boundariesAriaLabel: '文件夹访问',
      noBoundaries: '暂无额外文件夹。',
...
commandPalette: {
  ...
  agent: {
    ...
    issue: {
      heading: 'Issue',
      panelAriaLabel: '智能体工作',
      viewsAriaLabel: 'Issue 视图',
      listAriaLabel: 'Issue 列表',
      issue: 'Issue',
      ...
      loading: '正在加载 Issue……',
      refresh: '刷新 Issue',
      activeSessions: ({ count }) => `${count} 个 Agent Session 活跃中`,
      notification: {
        complete: '完成',
        error: '需要处理',
        canceled: '已取消',
        open: ({ status, title }) => `打开 Issue 详情：${title}（${status}）`,
      },
      view: {
        inbox: '收件箱',
        today: '今天',
    ...
    issueDetail: {
      ...
      backToParent: '返回上级 Issue',
      breadcrumbAriaLabel: 'Issue 路径',
      instructions: '指令',
      latestResult: '最新结果',
      noSessionActivity: '暂无 Agent Session 动态。',
      nextRun: '下次运行',
      process: '过程',
      execution: '执行',
      sessionNotFound: '未找到 Agent Session。',
      sessionTitle: 'Agent Session',
      starts: '开始',
      sessions: 'Agent Sessions',
      transcript: '会话记录',
      childIssues: '子 Issue',
      generatedIssues: '生成的 Issue',
      activity: '动态',
      noActivity: '暂无动态。',
      noExecutionDetails: '暂无执行详情。',
      acceptReview: '接受并完成',
      acceptingReview: '正在完成...',
      reviewFailed: '无法完成人工审核。',
      activityEvent: {
        created: '已创建。',
        updated: '已更新。',
        updatedFields: ({ fields }) => `已更新 ${fields}。`,
        archived: '已归档。',
        deleted: '已删除。',
        changedField: ({ field }) => `已更改 ${field}。`,
        changedStatus: ({ status }) => `状态已更改为 ${status}。`,
        action: {
          agentSessionStarted: 'Agent Session 已启动。',
          agentSessionStopped: 'Agent Session 已停止。',
          childIssueCreated: '子 Issue 已创建。',
          childIssueCompleted: '子 Issue 已完成。',
          skippedNextOccurrence: '已跳过下次发生。',
          generatedIssue: 'Issue 已生成。',
        },
        verification: {
          pass: ({ body }) => `验证通过：${body}`,
          fail: ({ body }) => `验证失败：${body}`,
          partial: ({ body }) => `部分通过：${body}`,
        },
      },
      activityMetadata: {
        time: '时间',
