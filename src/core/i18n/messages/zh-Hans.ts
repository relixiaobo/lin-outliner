...
export const zhHans: PartialMessages = {
  ...
  settings: {
    ...
    permissions: {
      sectionAriaLabel: '安全',
      trustLevelGroup: '信任级别',
      trustLevelAriaLabel: '信任级别',
      trustLevelLabel: '全局信任',
      trustLevelSublabel: '选择常规智能体工作的默认姿态。下面的例外会覆盖这个默认值，但不能覆盖硬性安全阻断。',
      askFirstMode: '先询问',
      balancedMode: '平衡',
      fullAccessMode: '完全访问',
      customModeLabel: '自定义',
      basedOnMode: ({ mode }) => `基于${mode}`,
      changedCount: ({ count }) => `已更改 ${count} 项`,
      resetToMode: ({ mode }) => `重置为${mode}`,
      precedenceFootnote: ({ mode }) => `顺序：硬性安全阻断，然后是你的例外，最后是${mode}默认值。`,
      exceptionsGroup: ({ mode }) => `${mode}的例外`,
      exceptionsAriaLabel: ({ mode }) => `${mode}的例外`,
      noExceptions: '没有例外。所有操作都跟随当前模式。',
      modifiedChip: '已修改',
      advancedRuleChip: '高级',
      revertException: '还原',
      addExceptionSummary: '添加例外',
      rawActionDescription: ({ action }) => `${action} 的操作规则。`,
      rawRuleDescription: '高级 JSON 规则，可能匹配多个目录操作。',
      acceptedSkillsGroup: '已接受的技能',
      acceptedSkillsAriaLabel: '已接受的技能',
      skillGrantSublabel: '已接受的技能哈希',
      revokeGrant: '撤销',
      commonActionsGroup: '操作目录',
      commonActionsAriaLabel: '操作目录',
    ...
    agents: {
      ...
      scaffoldBody: '你是一个专注的子智能体。\n完成任务后只汇报关键结果。',
      modelPlaceholder: 'inherit',
      effortDefault: '继承',
      permissionInherit: '跟随全局',
      trusted: '信任',
...
commandPalette: {
  ...
  agent: {
    chat: {
      ...
      createChannel: '创建频道',
      createChannelWithAgent: ({ name }) => `与 ${name} 创建频道…`,
      directMessages: '私信',
      ...
      createdFromDmNotice: ({ name }) => `由你与 ${name} 的私信创建 · 私信历史未共享`,
      ...
      addMember: '添加智能体到频道',
      inspectMemberPov: ({ name }) => `查看 ${name} 的 POV`,
      povInspectorAriaLabel: ({ name }) => `${name} 的组装 POV`,
      povInspectorTitle: ({ name }) => `${name} 的 POV`,
      povInspectorBoundary: ({ messageId }) => `最近被点名消息：${messageId}`,
      povInspectorNoBoundary: '完整频道转录',
      closePovInspector: '关闭 POV 检查器',
      povInspectorMemory: '记忆 briefing',
      povInspectorNoMemory: '该成员暂无记忆 briefing 分区。',
      povInspectorMessages: '组装后的消息',
      povInspectorNoMessages: '暂无可见消息。',
      povInspectorEmptyPart: '空文本片段',
      removeMember: '从频道移除',
      removeMemberWhileActive: '频道运行中不能移除成员',
      noAddableAgents: '暂无其他可添加的智能体',
    ...
    composer: {
      ...
      fullAccessConfirm: '要把 Lin 切换到完全访问吗？常规本地自动化和外部变更可以不再询问，但硬性安全阻断仍然生效。',
      denyOnce: '拒绝一次',
      acceptSkill: '接受技能',
      notNow: '暂不',
      dismiss: '知道了',
