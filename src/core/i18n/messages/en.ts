...
export const en = {
  ...
  settings: {
    ...
    permissions: {
      ...
      trustLevelGroup: 'Trust Level',
      trustLevelAriaLabel: 'Trust level',
      trustLevelLabel: 'Global trust',
      trustLevelSublabel: 'Choose the default posture for routine agent work. Exceptions below override this default, never hard safety blocks.',
      askFirstMode: 'Ask First',
      balancedMode: 'Balanced',
      fullAccessMode: 'Full Access',
      customModeLabel: 'Custom',
      basedOnMode: ({ mode }: { mode: string }) => `Based on ${mode}`,
      changedCount: ({ count }: { count: number }) => `${count} changed`,
      resetToMode: ({ mode }: { mode: string }) => `Reset to ${mode}`,
      precedenceFootnote: ({ mode }: { mode: string }) => `Order: hard safety blocks, then your exceptions, then the ${mode} default.`,
      exceptionsGroup: ({ mode }: { mode: string }) => `Exceptions to ${mode}`,
      exceptionsAriaLabel: ({ mode }: { mode: string }) => `Exceptions to ${mode}`,
      noExceptions: 'No exceptions. Every action follows the current mode.',
      modifiedChip: 'Modified',
      advancedRuleChip: 'Advanced',
      revertException: 'Revert',
      addExceptionSummary: 'Add an exception',
      rawActionDescription: ({ action }: { action: string }) => `Action rule for ${action}.`,
      rawRuleDescription: 'Advanced JSON rule; it can match more than one catalog action.',
      acceptedSkillsGroup: 'Accepted Skills',
      acceptedSkillsAriaLabel: 'Accepted skills',
      skillGrantSublabel: 'Accepted skill hash',
      revokeGrant: 'Revoke',
      commonActionsGroup: 'Action Catalog',
      commonActionsAriaLabel: 'Action catalog',
      ...
      followModeOption: (_: { mode: string; decision: string }) => 'Default',
      askOption: 'Ask first',
      allowOption: 'Always allow',
      denyOption: 'Always deny',
...
commandPalette: {
  ...
  agent: {
    ...
    chat: {
      ...
      createChannelWithAgent: ({ name }: { name: string }) => `Create a Channel with ${name}…`,
      ...
      createdFromDmNotice: ({ name }: { name: string }) => `Created from your DM with ${name} · DM history not shared`,
    ...
    composer: {
      ...
      fullAccessConfirm: 'Switch Lin to Full Access? Routine local automation and external mutations can run without asking, while hard safety blocks still apply.',
      denyOnce: 'Deny once',
