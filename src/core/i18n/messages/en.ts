  ...
  settings: {
    ...
    agent: {
      ...
      delegation: {
        ...
        runner: 'Runner',
        defaultRunner: 'Default launcher',
    ...
    general: {
      ...
      memoryResetTargetCounts: ({ containers, nodes, ordinary }: { containers: number; nodes: number; ordinary: number }) => `Containers: ${containers}. Nodes: ${nodes}, including ordinary notes: ${ordinary}.`,
      memoryResetPending: 'Reset is awaiting settlement.',
      memoryResetConflicted: 'Memory changed. Reset was not applied.',
      memoryResetUnknown: 'Reset outcome is unknown.',
      memoryNavigationUnavailable: 'Memory search is saved, but navigation was not confirmed.',
      memoryPreferenceSaved: 'Preference saved. Application is pending.',
      ...
      websiteDataUsage: ({ bytes, previews }: { bytes: number; previews: number }) => `${bytes.toLocaleString()} cache bytes; ${previews} open previews`,
      ...
      translationDataUsage: ({ entries, bytes }: { entries: number; bytes: number }) => `${entries.toLocaleString()} saved passages; ${bytes.toLocaleString()} logical bytes`,
