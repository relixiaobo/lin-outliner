  ...
  agent: {
    ...
    thread: {
      ...
      activity: {
        ...
        commandFailed: ({ command }: { command: string }) => `Command failed · ${command}`,
        ...
        fileChangeFailed: ({ count }: { count: number }) => `Failed to change ${count} ${count === 1 ? 'file' : 'files'}`,
        usingTool: ({ name }: { name: string }) => `Using ${name}`,
        usedTool: ({ name }: { name: string }) => `Used ${name}`,
        toolFailed: ({ name }: { name: string }) => `${name} failed`,
    ...
    turnDetails: {
      ...
      request: 'Request',
      response: 'Response',
      preAdapterContext: 'Pre-adapter context',
      preAdapterContextHint: 'Tenon projection passed to the provider adapter',
      systemInstructions: 'System instructions',
      preparedTools: ({ count }: { count: number }) => `Tool definitions (${count})`,
      preparedMessages: ({ count }: { count: number }) => `Messages (${count})`,
      providerRequest: 'Provider Request Content',
      modelResponse: 'Model Response',
      providerRequestJson: 'Provider request JSON',
      normalizedModelResponseJson: 'Normalized model response JSON',
      ...
      headersReceivedAt: 'HTTP headers received at',
      timeToHeaders: 'Time to HTTP headers',
      assistantResponseCompletedAt: 'Assistant response completed at',
      ...
      reportedCacheRead: 'Reported cache read',
      reportedCacheWrite: 'Reported cache write',
      reportedCacheWrite1h: 'Reported 1h cache write',
      reportedReasoningTokens: 'Reported reasoning tokens',
      reportedTotalTokens: 'Reported total tokens',
      calculatedCost: 'Calculated cost',
      providerError: 'Provider error',
      requestFingerprint: 'Provider request fingerprint',
      cacheBreakpoints: 'Cache breakpoints',
      ...
      protectedBoundary: 'Protected canonical message index',
      commonPrefixMessages: 'Canonical common-prefix messages',
      profile: 'Profile',
      ...
      copyRequestDiagnostics: 'Copy request diagnostics',
      requestDiagnosticsCopied: 'Request diagnostics copied',
