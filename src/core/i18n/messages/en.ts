...
export const en = {
  startup: {
    dataRecovery: {
      ...
      description: 'Verified local backups preserve your workspace, conversations, Profile, Memory and task history.',
      backup: 'Create backup', restore: 'Restore', retry: 'Retry inspection',
      resume: 'Enable future automatic work', cancel: 'Cancel', refresh: 'Refresh backups',
      exportDiagnostics: 'Export recovery details', empty: 'No completed backups yet.',
      invalid: 'Backup verification failed', backups: 'Verified backups',
      retained: 'Retained originals',
      retainedDetail: 'These originals could not be validated as a restorable dataset. Keep them for inspection or repair.',
      reveal: 'Show retained files', cancelOperation: 'Cancel pending operation',
      repair: 'Rebuild conversation views', restartRepair: 'Restart and rebuild conversation views?',
      repairDetail: ({ threads }: { threads: number }) => `Verified event sources for ${threads} conversations. The original projection will be retained before its views are rebuilt.`,
      working: 'Checking recovery options…',
      supersedeDetail: 'This will replace the pending recovery operation. Its retained data and journal will remain available.',
      paused: 'Restored automatic work is paused',
      pausedDetail: 'Historical tasks and publications remain available for inspection. Enabling future work will not replay their old execution claims.',
      restartBackup: 'Restart to create a verified backup?',
      restartRestore: 'Restart and restore the selected backup?',
      restartRetry: 'Restart to retry data inspection?',
      restartResume: 'Restart and enable future automatic work?',
      restart: 'Restart',
      backupDetail: 'Tenon will finish closing its writers before taking the backup.',
      restoreDetail: 'Your current data will be retained first. Restored automatic tasks will stay paused, and this will not undo changes on other devices.',
      phase: { inspecting: 'Inspecting data…', backingUp: 'Creating backup…', migrating: 'Preparing compatible data…', restoring: 'Restoring data…',
        rebuilding: 'Rebuilding derived data…', ready: 'Data is ready', recoveryRequired: 'Data needs attention' },
      details: ({ files, megabytes }: { files: number; megabytes: string }) => `${files} files · ${megabytes} MB`,
      unavailable: 'Data recovery is unavailable in this window.',
    },
    ...
    threadFailed: 'Conversation unavailable',
    quarantined: 'History could not be read',
    dependentThreads: 'Unavailable because an ancestor could not be read',
    quarantineHelp: 'Saved data is preserved. Other conversations remain available. Inspect recovery options for this conversation.',
    recovery: {
      inspect: 'Inspect recovery options', inspecting: 'Inspecting recovery options...',
      rebuild: 'Rebuild conversation', remove: 'Remove conversation', resume: 'Continue recovery',
      reinspect: 'Review updated scope', reveal: 'Show retained originals', cancel: 'Cancel',
      scope: 'Affected conversations', retained: 'Retained originals', resources: 'Referenced resources',
      preserved: 'Other conversations, independent forks, Projects, settings, accepted Profile information, Memory notes and shared attachments are preserved.',
      confirmRemove: 'Remove only these conversations after retaining their original evidence?',
      confirmRebuild: 'Rebuild this conversation from its verified retained source?',
      sourceRollout: 'Verified source: original conversation events',
      sourceProjection: 'Verified source: retained conversation snapshot. Original event order and missing attachments cannot be recreated.',
      retaining: 'Retaining original evidence...', applying: 'Recovering conversation...', complete: 'Recovery completed.',
      cancelled: 'Recovery cancelled.', blocked: 'Recovery is unavailable until these issues are resolved:',
      failed: 'Recovery could not finish. Your retained evidence and pending operation remain available.',
    },
    copyDetails: 'Copy details',
    copied: 'Details copied',
    openSource: 'Open configuration file',
    continue: 'Continue with notes',
    notesAvailable: 'Your notes are ready to use.',
    issues: 'Startup issues',
    agentStarting: 'Starting conversations...',
    quit: 'Quit',
  },
  ...
  settings: {
    discovery: {
      ...
      noModified: "No preferences have been customized.",
  ...
  minimumValue: ({ minimum }: { minimum: number }) => `Minimum: ${minimum}.`,
  valueRange: ({ minimum, maximum }: { minimum: number; maximum: number }) => `Allowed range: ${minimum}–${maximum}.`,
  "automatic": "Default",
  "system": "System",
  "none": "None",
  "short": "Short",
  ...
  "fields": {
    ...
    providers: {
      ...
      defaultImageModelUnavailable: 'The saved model is unavailable. Image requests will use automatic selection.',
      imageModelAuto: 'Automatic',
    ...
    security: {
      ...
      fullAccessBoundaryNote: 'The agent reaches whatever your macOS account reaches, including Tenon\u2019s own data and stored provider credentials. macOS privacy permissions, administrator approval, Keychain controls, and service sign-in still apply.',
    },
    skills: {
      sectionAriaLabel: 'Skills',
      availabilityHint: 'Choose which skills are available in Tenon. An agent’s custom selection can restrict this list further.',
      ...
      curationReport: 'Check Skill Files…',
      curationRunning: 'Checking…',
      curationTitle: 'Skill File Diagnostics',
      curationDescription: 'Check Skills written by the Agent for missing files, duplicate content, and outdated tool names. Built-in, installed, and manually edited Skills are skipped. Files stay unchanged.',
      curationSummary: ({ included, excluded, findings }: { included: number; excluded: number; findings: number }) =>
        `${included} checked, ${excluded} skipped, ${findings} issue${findings === 1 ? '' : 's'}`,
      curationIncluded: 'Checked',
      curationExcluded: 'Skipped',
      curationNoFindings: 'No file problems found.',
      curationClose: 'Done',
      sourceLocal: 'local',
      ...
      managedChecking: 'Checking for updates…',
      managedUpToDate: 'Up to date',
      managedCheckSummary: ({ updates, failed }: { updates: number; failed: number }) => updates === 0 && failed === 0 ? 'All Skills are up to date.' : [updates ? `Updates available: ${updates}.` : '', failed ? `Could not check: ${failed}.` : ''].filter(Boolean).join(' '),
      ...
      managedInstalledNotice: ({ name }: { name: string }) => `${name} installed.`,
      ...
      managedErrorCancelled: 'Skill operation cancelled.',
      managedErrorReviewExpired: 'The review expired. Inspect the Skill again.',
      managedErrorInteractionUnavailable: 'The Skill review window is unavailable.',
      managedErrorOperationUnavailable: 'The Skill or its permissions changed. Inspect it again.',
    },
    footer: {
      cancel: 'Cancel',
      save: 'Save',
    ...
    agent: {
      ...
      delegation: {
        label: 'Delegation',
        ariaLabel: 'Agent delegation',
        experimental: 'Experimental delegation',
        runner: 'Runner',
        defaultRunner: 'Default runner',
        runnerOptionsHint: ({ runner }: { runner: string }) => `The model, access, and time limit below apply to ${runner}.`,
        runnerEnabled: 'Runner enabled',
        internalRunner: 'Internal Runner',
        codexRunner: 'Codex CLI',
        claudeRunner: 'Claude CLI',
        openclawRunner: 'OpenClaw CLI',
        codexDetected: 'Codex CLI · Detected',
        codexReady: 'Codex CLI · Ready',
        codexUnavailable: 'Codex CLI · Not ready',
        launcherDetected: 'CLI detected, but not ready',
        launcherReady: 'Native CLI · Ready',
        launcherUnavailable: 'Native CLI · Not found',
        runnerReady: 'Internal · Ready',
        runnerDisabled: 'Internal · Disabled',
        model: 'Model',
        reasoning: 'Reasoning',
        inheritParent: 'Inherit parent',
        nativeLauncherModel: 'The native CLI controls its model and reasoning settings.',
        modelUnavailable: 'The selected model is unavailable. New delegated Sessions will be refused.',
        unavailable: 'Unavailable',
        maximumAccess: 'Maximum access',
        readOnly: 'Read only',
        workspaceWrite: 'Workspace write',
        turnDuration: 'Turn duration',
        minutes: ({ count }: { count: number }) => count < 60 ? `${count} min` : `${count / 60} hr`,
        advanced: 'Delegation limits',
        globalConcurrent: 'Running globally',
        threadConcurrent: 'Running per Thread',
        runnerConcurrent: 'Running on Runner',
        poolConcurrent: 'Running in pool',
        globalQueue: 'Queued globally',
        threadQueue: 'Queued per Thread',
        saved: 'Delegation settings saved',
        saveFailed: 'Delegation settings could not be saved.',
      },
    },
    agents: {
      profileFiles: 'Personal context',
      profileFilesHint: 'Identity and style belong to this Agent profile. Your personal context is shared across profiles. Accepted edits apply to the next conversation turn.',
      profileIdentity: 'Identity',
      profileStyle: 'Style',
      profileUser: 'About you',
      profileMissing: 'Using defaults',
      profileAccepted: 'Accepted · applies next turn',
      profileRejected: 'Needs correction',
      profileSelected: 'Selected for the latest turn',
      profileRevisions: ({ accepted, selected }: { accepted: number; selected: number | null }) => `Accepted revision: ${accepted}. Last selected revision: ${selected ?? 'none'}.`,
      profilePending: 'Pending',
      profileRefresh: 'Refresh status',
      profileSource: 'View evidence',
      profileUnavailable: 'The original evidence is unavailable. The accepted source reference is retained.',
      profileTruncated: 'This preview is limited to the first 16,000 characters.',
      profileManual: 'File edit',
      profileAgent: 'Edited in conversation',
      profileLearned: 'Learned from conversation',
      profileEntryHelp: 'Use a stable entry key, a Scope line, and a complete statement. Editing a learned entry protects it from automatic replacement. Delete its section to forget it.',
      profileSaved: 'Accepted. This edit applies on the next turn.',
      ...
      builtInFootnote: 'Customize your agent’s name, appearance, standing instructions, and available tools and skills.',
      editAction: 'Edit…',
      ...
      capabilitiesMainFootnote: 'These limits also apply to work this agent delegates.',
      allTools: 'All available',
      followSkillLibrary: 'Follow Skill Library',
      customSelection: 'Custom selection',
      skillSelectionHint: 'Skills turned off in the Skill Library remain unavailable.',
      skillOffInLibrary: 'Off in Skill Library',
      skillStatusUnavailable: 'Skill Library status is unavailable. Saved selections are unchanged.',
      searchCapabilities: ({ name }: { name: string }) => `Search ${name.toLowerCase()}`,
      selectAll: 'Select All',
      deselectAll: 'Deselect All',
      noCapabilities: 'No matching items.',
    ...
    general: {
      ...
      memorySublabel: 'Remember useful information from conversations. Saved memories can be viewed and edited in Daily Notes.',
      memoryStatusLabel: 'Saved memories',
      memoryReady: 'Ready to save useful information after conversations become idle.',
      memoryPaused: 'Memory is off. New memories will not be saved.',
      ...
      memoryResetSublabel: 'Permanently delete all Memory entries and their nested notes. Notes outside these entries are kept.',
      memoryResetAction: 'Reset Memory',
      memoryResetConfirmTitle: 'Reset Memory',
      memoryResetConfirmMessage: 'Permanently delete all Memory entries and every note nested inside them, including notes you added yourself? Other notes are kept, and old conversations will not restore deleted Memory.',
      ...
      memoryProfileResetPartial: 'Learned profile entries were removed; the reviewed note reset has not completed.',
      memoryResetTargetCounts: ({ containers, nodes, ordinary, profiles = 0 }: { containers: number; nodes: number; ordinary: number; profiles?: number }) => `Memory sections: ${containers}. Notes to delete: ${nodes}, including ${ordinary} ordinary notes. Learned profile entries: ${profiles}. Authored profile entries and identity/style files are preserved.`,
      memoryResetPending: 'Confirming whether Memory was reset. Please wait before trying again.',
      memoryResetConflicted: 'Memory changed. Reset was not applied.',
      memoryResetUnknown: 'Reset outcome is unknown.',
      memoryNavigationUnavailable: 'Could not confirm that the Memory view opened. Try Open Memory again.',
      memoryPreferenceSaved: 'Saved. Waiting for Memory to finish applying the change.',
      ...
      websiteDataReloadFailed: 'Website data cleared, but a preview could not be reloaded.',
      websiteDataUsage: ({ size, previews }: { size: string; previews: number }) => `${size} cached · ${previews} open previews`,
      translationDataGroup: 'Translation data',
      previewDataRetry: 'Retry',
      ...
      translationContentClearConfirmMessage: 'Clear saved translations for this content in all languages and models?',
      translationContentClearAction: 'Clear cached translations for this content',
      ...
      translationDataUsage: ({ entries, size }: { entries: number; size: string }) => `${entries.toLocaleString('en')} saved passages · ${size}`,
    ...
    shortcuts: {
      ...
      assign: ({ name }: { name: string }) => `Set shortcut for ${name}`,
  ...
  agent: {
    ...
    automations: {
      work: {
        ...
        changed: 'Saved task changed',
        reload: 'Reload saved task',
        ...
        attentionHistory: 'Other runs need attention',
        awaitingRun: 'Results will appear here after the first run.',
        pausedHint: 'The schedule is paused. You can still run this task now.',
        runningHint: 'This task is working. Its result will appear here.',
        waitingHint: 'This task is waiting to start.',
        stoppingHint: 'Stopping the current run…',
        unavailableHint: 'The saved execution record is unavailable.',
        clearSearch: 'Close search',
        attentionHint: 'Needs your attention',
        ...
        "missed": "Time passed while Tenon was unavailable.",
        "fulfill": "Run this time",
        "skip": "Skip this time",
        "back": "Back to tasks",
        "emptyResult": "No result yet.",
        "local": "Runs require Tenon and the required local resources to be available.",
      ...
      editor: {
        ...
        editTask: 'Edit',
        runUnavailable: 'This run’s conversation is unavailable.',
        noRuns: 'This task has not run yet.',
        nameRequired: 'Give this task a name.',
        namePlaceholder: 'e.g. Morning news brief',
        runOnce: 'Run once',
        saveRun: 'Save and run once',
        viewRun: 'View current run',
        savedRunFailed: 'Task saved. Run could not be confirmed; retrying will reuse the same request.',
        removeProject: 'Remove task project',
        runningDraft: 'Your edits are not saved yet. Return here to continue editing.',
        task: 'Task',
        placeholder: 'Describe the task. Use @ to reference content.',
        addReference: 'Reference content',
        chooseFiles: 'Choose files',
        pickerUnavailable: 'The file picker is unavailable.',
        useSavedFile: 'Use @ or Add attachment to reference a saved local file.',
        longPaste: 'This text is too large for the task. Reference a saved file instead.',
        unavailableReference: 'Unavailable content',
        when: 'When',
        options: 'More options',
        defaults: 'Use default',
        folder: 'Work in folder',
        chooseFolder: 'Choose folder…',
        defaultFolder: 'Default folder',
        separateCopy: 'Use a separate Git working copy',
        copyHelp: 'Edits go into a separate Git working copy. Existing access rules still apply.',
        paused: 'Paused · Saving keeps this task paused.',
        future: 'Changes apply to future runs. A run already in progress keeps its instructions.',
        previewLoading: 'Checking next run…',
        previewInvalid: 'Choose a valid future time.',
        ifResumed: 'If resumed: ',
        next: 'Next run: ',
        custom: 'Saved custom schedule',
        customHelp: 'Other edits keep this schedule unchanged.',
        replaceSchedule: 'Change schedule',
        replaceTitle: 'Replace the saved schedule?',
        replaceHelp: 'The saved custom schedule cannot be fully shown in this editor. Replace it with the settings shown below?',
        replace: 'Replace schedule',
        keepSchedule: 'Keep schedule',
        timezone: 'Change timezone',
        searchZone: 'Search time zones',
        local: 'Runs on this device while Tenon is available. Results return to this task.',
        save: 'Save changes',
        taskRequired: 'Describe the work you want done.',
        referenceSettings: 'Reference options',
        attached: 'Attached context',
        continueMissing: 'Continue if unavailable',
        requiredHelp: 'Unavailable required content blocks dependent work. Optional content may be skipped.',
        remove: 'Remove',
        saved: 'Saved version',
        yours: 'Your draft',
        reviewSaved: 'Review saved version',
        applyDraft: 'Save my draft against this version',
        applyHelp: 'Only do this after reviewing all differences. This replaces the saved definition with your draft.',
        pauseSaved: 'Pause saved schedule',
        actions: 'Saved schedule actions',
        model: 'Model',
        reasoning: 'Thinking effort',
        name: 'Name',
        folderMissing: 'Choose a working folder.',
        isolationChange: 'The separate Git copy setting is still enabled. Choose a Git folder or turn it off explicitly.',
        copy: 'Separate Git copy',
        pausedNext: 'If resumed, next run',
        changed: 'This task changed elsewhere. Your draft is preserved.',
        emptyReferences: 'No additional context',
        sourceLimit: 'A task supports up to 32 distinct references.',
        monthly: 'Every month',
        yearly: 'Every year',
      },
      title: 'Scheduled tasks',
      open: 'Open Scheduled tasks',
      ...
      unread: 'Unread run',
      filters: { all: 'All', active: 'Active', paused: 'Paused', completed: 'No future time' },
      ...
      discardConfirm: 'Your unsaved task changes will be lost.',
    ...
    thread: {
      ...
      waitingOnUserInput: 'Waiting for your answers',
      ...
      tasks: {
        ...
        launch: 'Launch',
        launchVerified: 'Verified and handed over',
        launchUnfinished: 'Not yet handed over',
        monitoring: 'Monitoring',
        watching: 'Watching on your request',
        notWatching: 'Off',
        available: 'Ready for use',
        verifyingLaunch: 'Awaiting startup verification',
        needsAttention: 'Task needs attention',
        exitCode: 'Exit code',
        signal: 'Signal',
        unknown: 'Unknown',
        stopRequest: 'Stop request',
        noKnownStop: 'None recorded; exit initiator unknown',
        stopSources: { user: 'Requested by you', agent: 'Requested by Agent', shutdown: 'Application shutdown', turnCancellation: 'Turn cancelled' },
        directory: 'Directory',
        ...
        clearDetailsTitle: 'Clear settled task details?',
        clearDetailsMessage: 'This permanently removes eligible output and artifacts from settled background tasks that no longer need delivery. Task results and handling records remain.',
      ...
      inputRemaining: ({ seconds }: { seconds: number }) => `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`,
      inputRemainingLabel: ({ seconds }: { seconds: number }) => `${seconds} seconds remaining`,
      inputRestoring: 'Loading questions…',
      inputRestoreError: 'Could not load the questions. Try again.',
      inputRetry: 'Try again',
      inputExpired: 'Time is up. No answers were submitted.',
      inputNotSubmitted: 'Not submitted',
      inputRetainedDraft: 'This draft was not submitted.',
      inputSkipAll: 'Skip all',
      inputSkipAllHint: 'Continue without submitting any answers. Keep your draft.',
      inputWriteAnswer: 'Your answer',
      inputDeadlineHint: 'When the wait ends, the agent continues without submitting your draft answers.',
      inputSendAnswers: 'Submit answers',
      inputSendAnswersHint: 'Submit your answers and skip unanswered questions.',
      inputSubmitting: 'Submitting…',
      inputSkipping: 'Skipping…',
      inputSubmitError: 'Could not submit your answers. Your draft is still here. Try again.',
      inputSkipError: 'Could not skip the questions. Your draft is still here. Try again.',
      inputSavedDraft: 'Answer draft',
      inputCancelled: 'Questions cancelled. Your draft is still here.',
      inputFailed: 'Could not complete the questions. Your draft is still here.',
      inputInvalidated: 'These questions are no longer available. Your draft is still here.',
      inputUnknown: 'Could not confirm whether your answers were submitted. Your draft is still here.',
      inputCheckingStatus: 'Checking answer status…',
      inputNeeded: 'Questions',
      inputForward: 'Next question',
      inputReplyPlaceholder: 'Reply…',
      ...
      inputProgress: ({ current, total }: { current: number; total: number }) => `Question ${current} of ${total}`,
      inputBack: 'Previous question',
      inputNext: 'Next',
      otherPlaceholder: 'Or write your own response…',
      item: {
        image: 'Image',
      ...
      activity: {
        ...
        askingQuestions: 'Questions for you',
        askedQuestions: 'Questions asked',
  ...
  providerConfig: {
    availableKeyHint: 'Uses your existing credential.',
    duplicateId: 'This provider already exists. Choose a different ID.',
    providerIdHint: 'A unique name, e.g. office-proxy.',
    authentication: 'Connect using',
    account: 'Account',
    advanced: 'Advanced',
    retry: 'Try Again',
    invalidUrl: 'Enter a complete URL starting with http:// or https://.',
    endpointRequired: 'Your provider or local server’s API address.',
    endpointOptional: 'Leave empty to use the default address.',
    savedKeyPlaceholder: 'Saved key',
    keyLength: ({ count }: { count: number }) => `${count} character${count === 1 ? '' : 's'}`,
    savedKeyHint: 'Paste a new key to replace it.',
    localKeyHint: 'Optional for local servers without authentication.',
