...
export const SCHEDULED_RESULT_STATES = ['waiting', 'running', 'completed', 'blocked', 'failed', 'interrupted', 'omitted', 'unavailable'] as const;
...
export function decodeScheduledRunResult(value: unknown): ScheduledRunResult {
  ...
  const keys = ['run', 'state', 'resultTurnId', 'answer', 'parts', 'answerTruncated', 'startedAt', 'finishedAt', 'recordPath', 'issue', 'issueKey', 'acknowledged'];
  ...
  return { ...row, parts, run: decodeAutomationRun(row.run) } as unknown as ScheduledRunResult;
}
