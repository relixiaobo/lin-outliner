...
const spawnAgentSchema = objectSchema({
  ...
  max_total_tokens: numberSchema('Optional per-child token cap within the shared Subagent tree pool.'),
...
const collaborationToolContracts: readonly ModelToolContract[] = [
  {
    ...
    description: 'Create a child Thread, resolve its Agent Role, and start its first Turn. Descendants share one host-owned tree token pool; max_total_tokens optionally caps this child\'s own contribution within it.',
    scope: 'anyThread',
    schemaOwner: 'core',
  ...
  },
  {
    ...
    description: 'Block without polling until a child reaches a terminal state or the sender receives steering. Returns batched terminal outcomes, including final results, plus the current child tree; returns immediately only when no child is running. Each agent view reports the shared pool tokensUsed and tokenBudget. Synthesize completed results instead of repeating covered work. To verify or debug a reported result, read or grep the child transcript at its transcriptPath with the file tools; it stays readable after the child stops or exhausts its budget.',
    scope: 'anyThread',
    schemaOwner: 'core',
