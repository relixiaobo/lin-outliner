...
function prompt(text: string): (context: TenonAgentToolPromptContext) => string {
  return (context) => [
    ...
    'Issues are durable work contracts for independently user-visible outcomes. Author the objective, scope, acceptance criteria, output, trigger, and verification policy; the Agent Session owns internal planning and execution detail.',
...
export const AGENT_ISSUE_TOOL_DEFINITIONS: readonly TenonAgentToolDefinition[] = ([
  ...
  },
  {
    ...
    descriptionText: 'Create one Issue or Recurring Issue for a durable work outcome. Parentage is derived from the creating Agent Session.',
    promptGuidanceText: 'Use when the durable definition does not exist yet. Write one complete Issue contract for the outcome: objective, scope, coverage, output shape, and verification expectations. Inside an Agent Session, create a child Issue only when a sub-outcome needs its own durable lifecycle or independent execution; runtime derives the parent automatically. Request-mode creation of a when-ready unattended Issue is a background handoff: runtime starts eligible work and delivers the terminal result to the immediate origin target (parent Agent Session for a child, visible conversation for a root). After creating that Issue, continue only when its result returns through runtime instead of starting or duplicating the same work in the current turn. Use preview for ambiguous or broad-scope creation.',
  ...
  },
  {
    ...
    descriptionText: 'Request one Agent Session execution or orchestration pass for an existing eligible Issue.',
    promptGuidanceText: 'Use for explicit retries, continuations, verification attempts, or direct requests to start an existing eligible Issue. This is not the normal path after issue_create: runtime starts newly created when-ready unattended Issues when eligible. Use detach=false only when the user explicitly wants the current conversation to wait for this existing Session.',
  ...
  },
  {
    ...
    descriptionText: 'Read bounded status, Activity, or latest output for one Agent Session.',
    promptGuidanceText: 'Use for bounded inspection or an explicit short wait on an existing Agent Session. Background Issue results arrive through runtime notifications; do not poll by default.',
  ...
  },
  {
    ...
    descriptionText: 'Send guidance into an active Agent Session.',
    promptGuidanceText: 'Use for guidance inside an existing active Session. Use issue_update when durable Issue definition changes.',
