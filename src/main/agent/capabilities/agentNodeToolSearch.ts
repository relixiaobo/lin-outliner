...
export function validateReferenceTargetIds(index: ProjectionIndex, targetIds: string[]): NodeToolIssue | null {
  ...
  if (missing) {
    return {
      ...
      instructions: 'Use node_search to locate the target id, then retry with a canonical [[node://uuid]] marker or the exact Node id.',
    };
  }
...
function validateRule(rule: SearchQueryRule): NodeToolIssue | null {
  if (FIELD_OPERAND_OPS.has(rule.op) && !rule.fieldDefId && rule.op !== 'HAS_FIELD' && rule.op !== 'OVERDUE') {
    return missingRuleOperand(rule.op, 'field:: field:<exact-uuid>');
  }
  if (rule.op === 'HAS_TAG' && !rule.tagDefId) return missingRuleOperand(rule.op, 'tag:: tag:<exact-uuid>');
  if (TARGET_OPERAND_OPS.has(rule.op) && !rule.targetId) return missingRuleOperand(rule.op, 'target:: [[node://uuid]]');
...
function operandInstructionForOp(op: QueryOp): string {
  ...
  if (op === 'DATE_OVERLAPS') return 'Use field:: field:<exact-uuid> and value:: YYYY-MM-DD/YYYY-MM-DD. DATE_OVERLAPS searches date field values only.';
  if (
    op === 'FIELD_IS'
    || op === 'FIELD_IS_NOT'
    || op === 'FIELD_CONTAINS'
    ...
    return 'Use field:: field:<exact-uuid> plus value:: ... for user-defined fields. For checkbox completion state, use DONE, NOT_DONE, TODO, or DONE_LAST_DAYS instead.';
  }
  if (op === 'HAS_TAG') return 'Use tag:: tag:<exact-uuid>.';
  if (op === 'LINKS_TO' || op === 'CHILD_OF' || op === 'DESCENDANT_OF' || op === 'DESCENDANT_OF_WITH_REFS' || op === 'OWNED_BY') {
    return 'Use target:: [[node://uuid]] or an exact Node id.';
  }
...
function referenceOperandInstruction(name: 'field' | 'tag' | 'target', op?: QueryOp): string {
  if (name === 'field') {
    const base = 'Use field:: field:<exact-uuid>. Plain field names such as "date", "done", or "Status" are not enough.';
  ...
  if (name === 'tag') return 'Use tag:: tag:<exact-uuid>. Plain tag names are not enough.';
  return `Use target:: [[node://uuid]] or an exact target node id. ${NODE_REFERENCE_GUIDANCE}`;
}
...
function nodeReference(index: ProjectionIndex, nodeId: string, label?: string): string {
  ...
  return formatNamedNodeReference(nodeId, label ?? (node ? nodeTitle(index, node) : nodeId));
}
