import { threadHistoryProjectionStoreSchema } from './ThreadHistoryProjectionStore.schema';
...
export class ThreadHistoryProjectionStore {
  ...
  constructor(path: string, database?: SqliteDatabase) {
    ...
    try {
      ...
      this.db.exec(threadHistoryProjectionStoreSchema);
    } catch (error) {
  ...
  private projectRollback(
    ...
    marker: ThreadHistoryRollbackMarker | ThreadHistoryRerunMarker,
  ): void {
...
function reconstructOpenItems(
  ...
  for (const entry of entries) {
    ...
    if (event.type === 'history/rollback' || event.type === 'history/rerun') {
      if (event.omittedTurnIds.includes(turnId)) {
        openItems.clear();
        turnStarted = false;
      }
      if (event.type === 'history/rerun' && event.replacement.turnId === turnId) {
        openItems.clear();
        turnStarted = true;
      }
      continue;
