    ...
    blocks.push({
      ...
      text: [
        ...
        ...(has('task_control') ? [
          ...
          '- Attribute observations to their exact Task/Item and observation time. Distinguish execution facts, whether the requested outcome holds, and unverified causal hypotheses. A matching PID/path/name/port or healthy existing object does not prove this execution created or controls it; PID reuse, stale output and detached children require owner evidence or an explicit unknown.',
          '- The same rule applies to files and remote operations: use recorded versions/hashes/receipts to establish association. Existing output may satisfy an availability request but cannot prove a requested new version was produced. Inspection creates no mutation, Stop, replacement or restart authority.',
          '- Absence of evidence does not prove absence of an effect. Equal before/after file hashes do not rule out a rewrite with identical bytes; a missing write receipt cannot prove no write occurred. A recovered remote receipt does not contradict the user having lost the original reply. Report only the association or effect that the observations establish.',
