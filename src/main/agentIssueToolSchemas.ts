...
const ISSUE_RELATION_SCHEMA = {
  ...
  description: 'Visible relationship between independently user-visible flat Issues. Do not use relations to model child Issues, checklist steps, coverage items, verification passes, or hidden workflow decomposition.',
  properties: {
    type: {
      type: 'string',
      enum: ['blocked-by', 'blocks', 'related', 'duplicate-of'],
      description: 'Relationship kind between the current Issue and another independent Issue. blocked-by is for true external blockers, not internal decomposition.',
    },
...
const ISSUE_FIELDS_SCHEMA = {
  ...
  properties: {
    ...
    relations: { type: 'array', items: ISSUE_RELATION_SCHEMA, description: 'Visible dependency and relationship links between independent Issues; never a hierarchy or breakdown mechanism.' },
...
const RECURRING_TEMPLATE_SCHEMA = {
  ...
  properties: {
    delegate: AGENT_REF_SCHEMA,
    relations: { type: 'array', items: ISSUE_RELATION_SCHEMA, description: 'Independent-Issue relations copied to generated Issues; never a hierarchy or breakdown mechanism.' },
    trigger: ISSUE_TRIGGER_SCHEMA,
