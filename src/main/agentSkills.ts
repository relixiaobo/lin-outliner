...
const DEFAULT_BUILT_IN_SKILLS: readonly BuiltInSkillInput[] = [{
  ...
  body: [
    ...
    '   - If the user says "break down", "steps", or "each X", encode that as required internal structure in the Issue definition unless they explicitly ask to manage each item as a separate durable row.',
    '   - A good Issue definition gives the later Agent Session enough scope, coverage, output, and verification guidance to make its own execution plan.',
    ...
    '   - Leave execution sequencing, per-item work allocation, retries, and intermediate verification records to the Agent Session that runs from this Issue snapshot.',
