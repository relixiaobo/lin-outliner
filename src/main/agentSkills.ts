...
const DEFAULT_BUILT_IN_SKILLS: readonly BuiltInSkillInput[] = [{
  ...
  name: 'goal-launching',
  description: 'Create and supervise persistent verified Issues from natural-language requests.',
  ...
  body: [
    'Use this workflow when the user wants Tenon to pursue a persistent objective as a verified Issue, not when the user only wants an immediate answer.',
    '',
    '1. Separate the objective from acceptance criteria.',
    '   - The objective is the work to pursue.',
    ...
    '',
    '2. Define durable work as an Issue, not as an ad hoc checklist or direct Run.',
    ...
    '   - Persistent user goals should keep a verification policy; do not treat one successful execution as automatic Issue completion.',
    '',
    '3. Start or observe execution through Agent Session tools.',
    '   - If the created or existing Issue should run immediately and the runtime authorization path allows it, use `agent_session_start` for one execution attempt.',
    ...
    '   - More scope requires updating the Issue through the runtime authorization path; do not silently widen an executing Session snapshot.',
    '',
    '4. Outcomes are Issue status, Agent Session status, Activity, and verification evidence.',
