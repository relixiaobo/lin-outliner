...
const DEFAULT_BUILT_IN_SKILLS: readonly BuiltInSkillInput[] = [{
  ...
  body: [
    'Use this workflow when the user wants Tenon to pursue a persistent objective as a verified Run, not when the user only wants an immediate answer.',
    '',
    ...
    '',
    '3. Keep the user on the loop.',
    '   - After launch, report the run id/name and the criteria being verified.',
    '   - Do not poll with `run_status` unless the user asks or you need a specific status decision; terminal notifications return outcomes automatically.',
    '   - Use `run_steer` for soft guidance and `run_amend` only when the objective, criteria, or budget genuinely changed.',
    '   - More scope requires stopping and re-spawning with a wider still-authorized scope, or escalating to the attended ask gate.',
    '',
    '4. Outcomes are verified result, blocked, budget exhausted, or stopped.',
    '   - Never claim completion from the worker result alone; rely on objective status and verifier outcome.',
