...
const DEFAULT_BUILT_IN_SKILLS: readonly BuiltInSkillInput[] = [{
  ...
  name: 'issue-planning',
  description: 'Plan flat verified Issues from natural-language durable-work requests.',
  ...
  body: [
    'Use this workflow when the user wants Tenon to define durable work as flat verified Issues, not when the user only wants an immediate answer.',
    '',
    '1. Choose the durable Issue boundary from user-visible outcomes.',
    '   - Default to one Issue per independently user-visible outcome the user would manage as its own row.',
    '   - For "query Beijing districts and Chengdu districts weather", create one Beijing Issue and one Chengdu Issue; district-level queries are internal execution plan items inside those Issues.',
    '   - If the user says "sub-issue", "break down", "steps", or "each X", treat that as required internal structure unless they explicitly ask to manage each item as a separate durable Issue.',
    '   - Never create Issues for checklist steps, districts/locations inside one report, files/pages to process, search targets, verification passes, retries, implementation phases, or other internal decomposition.',
    '   - Do not simulate hierarchy with `relations`, `blocked-by`, title text like "sub issue", or descriptions like "as child of". Relations are only for independent Issues that already have their own user-visible lifecycle.',
    '',
    ...
    '   - Include the internal breakdown, coverage list, verification expectations, and output shape in the Issue description and criteria.',
    ...
    '   - Use an immediate/when-ready trigger when the user wants work to start now; use a scheduled or recurring trigger only when the user asked for time-based work.',
    ...
    '   - If an existing manual Issue should run now, use `agent_session_start` for one execution attempt.',
    '   - If a newly created Issue has an immediate, when-ready, scheduled, or recurring trigger, rely on runtime eligibility and scheduler behavior instead of manually creating child work.',
