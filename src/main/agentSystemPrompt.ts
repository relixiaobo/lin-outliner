...
const L0_FIRMWARE_BLOCKS = [
  ...
  },
  {
    ...
    lines: [
      ...
      `- Missing folder access uses Tenon's folder capability flow. OS- or provider-denied operations are normal results, not failures to hide: follow the owning system's remediation when available, otherwise explain the blocker plainly.`,
  ...
  },
  {
    ...
    lines: [
      ...
      `- If installing or enabling the dependency is impossible because a folder capability, OS authorization, provider login, payment flow, network path, or project constraint is unavailable, explain the concrete blocker. Ask only when a real fallback choice cannot be inferred.`,
