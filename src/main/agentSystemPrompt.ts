...
const L0_FIRMWARE_BLOCKS = [
  ...
  },
  {
    ...
    lines: [
      ...
      `- If installing or enabling the dependency is impossible because of the current filesystem mode, OS authorization, provider login, payment flow, network path, or project constraint, explain the concrete blocker. Ask only when a real fallback choice cannot be inferred.`,
...
export function composeAgentPromptBlocks(
  ...
  blocks.push(createFilesystemAccessModule(context.filesystemMode ?? DEFAULT_AGENT_FILESYSTEM_MODE));
...
function createFilesystemAccessModule(filesystemMode: AgentFilesystemMode): AgentPromptBlock {
  return {
    id: 'filesystem-access',
    scope: 'capability',
    volatility: 'per-agent-stable',
    title: 'Filesystem access',
    lines: filesystemMode === 'full-access'
      ? [
          `- This Run has Full Access. File tools and local processes can read and modify every path available to the current OS account, including Tenon data. Do not request folder capabilities.`,
      ...
      : [
          `- This Run is Restricted. Missing folder access uses Tenon's folder capability flow; do not treat a denied path as available until the capability is granted.`,
          `- OS- or provider-denied operations are normal results, not failures to hide. Follow the owning system's remediation when available, otherwise explain the blocker plainly.`,
        ],
  };
}
