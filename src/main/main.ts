...
function registerIpc() {
  ipcMain.handle('lin:invoke', async (_event, command: string, args?: Record<string, unknown>) => {
    const dispatch = () => {
      ...
      if (isPreviewCommand(command)) return handlePreviewCommand(command, args ?? {});
...
async function handlePreviewCommand(command: PreviewCommand, args: Record<string, unknown>) {
  ...
  switch (command) {
    case 'preview_resolve_source':
      return { source: await previewSourceForTarget(target) } satisfies PreviewResolveSourceResult;
    case 'preview_read_text':
      return previewTextForTarget(target);
    case 'preview_read_bytes':
      return previewBytesForTarget(target);
    case 'preview_list_directory':
      return previewDirectoryEntriesForTarget(target);
    default:
      throw new Error(`Unknown preview command: ${command}`);
  }
}

async function previewSourceForTarget(target: PreviewTarget): Promise<PreviewSourceDescriptor | null> {
  if (target.kind === 'local-file') {
    const file = await resolveTrustedLocalFileReference(target.path, [agentLocalFileRoot]);
    if (!file) return null;
    const metadata = await localFileReferencePreview(file);
  ...
  if (target.kind === 'asset') {
    const [metadata, filePath] = await Promise.all([
      assetService.lookup(target.assetId),
      assetService.pathFor(target.assetId),
    ]);
  ...
  if (target.kind === 'agent-payload') {
    const payload = await agentRuntime.previewPayload(target.conversationId, target.payloadId);
...
async function previewTextForTarget(target: PreviewTarget): Promise<PreviewReadTextResult> {
  const result = await previewBytesBufferForTarget(target, PREVIEW_TEXT_BYTE_LIMIT);
  if ('error' in result) return { text: null, error: result.error };
  return { text: result.bytes.toString('utf8') };
}

async function previewBytesForTarget(target: PreviewTarget): Promise<PreviewReadBytesResult> {
  const result = await previewBytesBufferForTarget(target, PREVIEW_BYTES_LIMIT);
...
async function previewBytesBufferForTarget(
  ...
  if (target.kind === 'agent-payload') {
    ...
    const bytes = await agentRuntime.previewPayloadBytes(target.conversationId, target.payloadId);
...
async function previewDirectoryEntriesForTarget(target: PreviewTarget): Promise<PreviewListDirectoryResult> {
  ...
  for (const dirent of dirents) {
    if (entries.length >= PREVIEW_DIRECTORY_ENTRY_LIMIT) break;
    const child = await resolveTrustedLocalFileReference(join(file.path, dirent.name), [agentLocalFileRoot]);
    if (!child) continue;
    const mimeType = child.entryKind === 'directory' ? 'inode/directory' : inferMimeType(child.path);
    entries.push({
