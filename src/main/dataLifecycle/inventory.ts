...
import { assertOwnedPath, missing, ownedPath } from './durableFiles';
...
export interface ManagedFile { readonly path: string; readonly kind: 'sqlite' | 'file'; readonly bytes: number }
export interface ManagedInventory { readonly roots: readonly ManagedRoot[]; readonly files: readonly ManagedFile[]; readonly excluded: readonly string[] }
export const MAX_BACKUP_FILES = 200_000;

export async function inspectManagedFiles(root: string, registry: DataStoreRegistry): Promise<ManagedInventory> {
  ...
  const visit = async (name: string): Promise<void> => {
    ...
    if (info.isSymbolicLink()) throw new Error('A managed data path is a symbolic link; retain its external source separately.');
    if (info.isDirectory()) {
    ...
    if (sidecar && registry.forPath(sidecar[1]!)) { excluded.push(name); return; }
    ...
    const applicationDatabase = name.startsWith('content/') || /^agent\/[^/]+\.sqlite$/.test(name);
    if (name.endsWith('.sqlite') && applicationDatabase && !registry.forPath(name)) throw new Error(`Unregistered database owner: ${name}`);
    files.push({ path: name, kind: registry.forPath(name) ? 'sqlite' : 'file', bytes: info.size });
