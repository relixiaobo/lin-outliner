...
export class AgentRuntimeStore {
  ...
  private providerRetry: AgentProviderRetryStatus | null = null;
  ...
  private handleEvent = (payload: AgentRuntimeEvent) => {
    ...
    if (payload.type === 'provider_retry') {
      ...
      if (payload.phase === 'retrying') {
        ...
        this.providerRetry = { runId, kind, attempt, maxRetries, timestamp };
        this.publish();
      } else if (this.providerRetry?.runId === payload.runId) {
        this.providerRetry = null;
        this.publish();
      }
      return;
