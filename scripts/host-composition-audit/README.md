...
Platform constructions are collected across the complete current tree and then
checked against a path-scoped manifest with explicit expected counts. The three
application window constructors, launcher constructor, and two retained Agent
web-capability windows remain distinct legitimate owners while any additional
managed construction still fails.
